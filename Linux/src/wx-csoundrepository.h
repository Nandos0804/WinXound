...
class wxCSoundRepository : public Gtk::EventBox // public Gtk::Frame
{
  ...
  protected:
    ...
    // void on_treeStructure_row_activated(const Gtk::TreeModel::Path& path, Gtk::TreeViewColumn*
    // column); bool on_treeStructure_clicked(GdkEventButton* event);
    void on_my_cursor_changed();

    void on_check_resize(Gdk::Rectangle &rect);

    void on_tree_drag_data_get(const Glib::RefPtr<Gdk::DragContext> &dragcontext,
