...
- (void)FillTreeViewOpcodes {
    ...
    for (NSString *mString in lines) {
        ...
        itemText = [[[split objectAtIndex:1] componentsSeparatedByString:@":"]
            objectAtIndex:0];

        // if (itemText.ToLower() == "utilities") continue;
        if ([[itemText lowercaseString] isEqualToString:@"utilities"])
            continue;
        if ([[itemText lowercaseString] isEqualToString:@"csoundav opcodes"])
            continue;
        ...
        // if([[[split objectAtIndex:1] componentsSeparatedByString:@":"] count]
        // > 1)
        if ([[split objectAtIndex:1] rangeOfString:@":"].location !=
            NSNotFound) {
            // string itemTextInside = split[1].Split(":".ToCharArray())[1];
            NSString *itemTextInside = [[[split objectAtIndex:1]
...
- (IBAction)codeInsertSynopsis:(id)sender {
    ...
    if ([opcodeValue length] > 0)
        text =
            [[items objectAtIndex:2] stringByReplacingOccurrencesOfString:@"\""
                                                               withString:@""];

    // Check text
    if (text == nil)
        return;
    if ([[text lowercaseString] rangeOfString:@"not available"].location !=
        NSNotFound)
        return;
