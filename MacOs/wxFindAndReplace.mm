...
- (IBAction)wxFindAndReplaceFIND:(id)sender {
    ...
    // NSLog(@"FIND");
    if ([[fieldFindText stringValue] length] < 1)
        return;

    [resultInfo setStringValue:@""];

    NSInteger ret = [textEditor
              FindText:[fieldFindText stringValue]
...
- (IBAction)wxFindAndReplaceREPLACEALL:(id)sender {
    ...
    [resultInfo setStringValue:@""];
    NSInteger ret = [textEditor
           ReplaceAllText:[fieldFindText stringValue]
            ReplaceString:[fieldReplaceText stringValue]
           MatchWholeWord:[[wxDefaults valueForKey:@"FindWholeWord"] boolValue]
                MatchCase:[[wxDefaults valueForKey:@"FindMatchCase"] boolValue]
        FromCaretPosition:[[wxDefaults valueForKey:@"ReplaceFromCaret"]
                              boolValue]
    ...
    if (ret <= 0) {
        ...
        [resultInfo
            setStringValue:[NSString
                               stringWithFormat:@"%d occurence(s) replaced",
                                                ret]];
    }
}
