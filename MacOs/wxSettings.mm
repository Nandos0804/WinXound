...
//--------------------------------------------------------------------------------------------------
// Useful Methods to retrieve lexer style information from database (for syntax
// highlight and style)
//--------------------------------------------------------------------------------------------------
- (NSString *)StyleGetFriendlyName:(NSString *)lang
    ...
    @try {
        NSString *s = [tempList
            valueForKey:[NSString stringWithFormat:@"%d", stylenumber]];
        if (s != nil) {
            // if (GetValues(s)[0] == Convert.ToString(stylenumber))
            if ([self GetValues:s] != nil)
            // if ([[[self GetValues: s] objectAtIndex:0] integerValue] ==
            // stylenumber)
            {
        ...
        NSLog(@"wxSettings -> StyleGetFriendlyName Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSString *)StyleGetForeColor:(NSString *)lang
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetForeColor Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSString *)StyleGetBackColor:(NSString *)lang
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetBackColor Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (BOOL)StyleGetBold:(NSString *)lang stylenumber:(NSInteger)stylenumber {
    tempList = [self SelectLanguage:lang];
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetBold Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (BOOL)StyleGetItalic:(NSString *)lang stylenumber:(NSInteger)stylenumber {
    tempList = [self SelectLanguage:lang];
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetItalic Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSInteger)StyleGetAlpha:(NSString *)lang stylenumber:(NSInteger)stylenumber {
    tempList = [self SelectLanguage:lang];
    if (tempList == nil)
        return 40;
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetAlpha Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (BOOL)StyleGetEolFilled:(NSString *)lang stylenumber:(NSInteger)stylenumber {
    tempList = [self SelectLanguage:lang];
    if (tempList == nil)
        return false;
    ...
    @try {
        ...
        NSLog(@"wxSettings -> StyleGetEolFilled Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSArray *)GetValues:(NSString *)s {
    @try {
        ...
        //										   "wxSettings.EditorProperties.GetValues
        //Error: " + ex.Message); return new string[] { "" }; return [[NSArray*
        // alloc]
...
- (NSColor *)HexToNSColor:(NSString *)inColorString {
    @try {
        ...
        NSLog(@"wxSettings -> HexToNSColor Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSString *)NSColorToHex:(NSColor *)color {
    ...
    @try {
        ...
        NSLog(@"wxSettings -> NSColorToHex Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (IBAction)wxSettingsSetDefaultTemplates:(id)sender {
    NSString *csoundTemplate = @"<CsoundSynthesizer>\n\n\n"
                                "<CsOptions>\n\n"
                                "</CsOptions>\n\n\n"
                                "<CsInstruments>\n\n"
                                "</CsInstruments>\n\n\n"
    ...
    NSString *cabbageTemplate = @"<Cabbage>\n"
                                 "</Cabbage>\n\n"
                                 "<CsoundSynthesizer>\n\n"
                                 "<CsOptions>\n"
                                 "-d -n\n"
...
- (IBAction)wxSettingsSetDefaultCompilerFont:(id)sender {
    ...
    [wxDefaults setValue:[NSNumber numberWithInteger:12]
                  forKey:@"CompilerFontSize"];
}
- (IBAction)wxSettingsSetDefaultEditorFont:(id)sender {
    ...
    [wxDefaults setValue:[NSNumber numberWithInteger:8]
                  forKey:@"EditorTabIndent"];
}
- (IBAction)wxSettingsSetDefaultEditorProperties:(id)sender {
    ...
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"SaveBookmarks"];
    [wxDefaults setValue:[NSNumber numberWithBool:false]
                  forKey:@"EditorMarkCaretLine"];
    [wxDefaults setValue:[NSNumber numberWithBool:false]
                  forKey:@"EditorShowVerticalRuler"];
}
- (IBAction)wxSettingsSetCSoundDefaultFlags:(id)sender {
    [wxDefaults setValue:@"-B4096 --displays --asciidisplay"
                  forKey:@"CSoundDefaultFlags"];
}
...
- (IBAction)wxAutoSearchPaths:(id)sender {
    ...
    if ([fileManager fileExistsAtPath:@"/usr/local/bin/csound"]) {
        [wxDefaults setValue:@"/usr/local/bin/csound"
                      forKey:@"CSoundConsolePath"];
    }
    ...
    if ([fileManager fileExistsAtPath:@"/Applications/Cabbage.app"]) {
        [wxDefaults setValue:@"/Applications/Cabbage.app"
                      forKey:@"CabbagePath"];
    }
    ...
    if (paths != nil) {
        ...
        NSString *quteCSoundPath = [NSString
            stringWithFormat:@"%@/QuteCsound.app", applicationDirectory];
        // NSLog(quteCSoundPath);
        // NSArray* files = [fileManager
        // contentsOfDirectoryAtPath:applicationDirectory error:nil];
        if ([fileManager fileExistsAtPath:quteCSoundPath]) {
            [wxDefaults setValue:quteCSoundPath
                          forKey:@"CSoundExternalGuiPath"];
        }
    }
    ...
    if (paths != nil) {
        ...
        NSString *manualPath =
            [NSString stringWithFormat:@"%@/Frameworks/CsoundLib.Framework/"
...
- (IBAction)wxSettingsFontPanelOK:(id)sender {
    // NSLog(@"OK");
    NSCell *cell = [fontTable preparedCellAtColumn:0
                                               row:[fontTable selectedRow]];
...
- (IBAction)wxShowFontPanel:(id)sender {
    ...
    if (tagSender == 0) {
        ...
        NSString *path =
            [[NSBundle mainBundle] pathForResource:@"compiler_example"
                                            ofType:@"txt"
                                       inDirectory:nil];
        [fontText
            setStringValue:[NSString
                               stringWithContentsOfFile:path
                                               encoding:NSUTF8StringEncoding
                                                  error:nil]];
        [fontSize setStringValue:[[wxDefaults valueForKey:@"CompilerFontSize"]
                                     stringValue]];
    } else {
        ...
        [fontText
            setStringValue:[NSString
                               stringWithContentsOfFile:path
                                               encoding:NSUTF8StringEncoding
                                                  error:nil]];
        [fontSize setStringValue:[[wxDefaults valueForKey:@"EditorFontSize"]
                                     stringValue]];
    }
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
    if ([[notification object] isEqualTo:fontTable]) {
        ...
        NSCell *cell = [fontTable preparedCellAtColumn:0
                                                   row:[fontTable selectedRow]];
        // NSFont* font = [NSFont fontWithName:[cell title] size:[[wxDefaults
        // valueForKey:@"EditorFontSize"] floatValue]];
        NSFont *font = [NSFont fontWithName:[cell title]
...
- (IBAction)wxResetDocumentSize:(id)sender {
    [wxDefaults setValue:[NSNumber numberWithInteger:950]
                  forKey:@"DocumentWindowWidth"];
    [wxDefaults setValue:[NSNumber numberWithInteger:700]
                  forKey:@"DocumentWindowHeight"];
}

- (IBAction)wxResetAllToDefault:(id)sender {
    // NSInteger ret = [wxMAIN ShowMessage:@"All values will be reverted to
    // factory defaults.\nWould you like to continue?" 					informativeText:@""
    //					  defaultButton:@"YES"
    //					alternateButton:@"NO"
    //						otherButton:@"Cancel"];
...
- (id)tableView:(NSTableView *)tableView
    ...
    @try {
        // FONT TABLE
        if ([tableView isEqualTo:fontTable]) {
            NSString *fontName = [[[NSFontManager sharedFontManager]
                availableFontFamilies] objectAtIndex:row];
            return fontName;
        }
        ...
        // SYNTAX TABLES
            ...
            if ([[[tableColumn headerCell] stringValue]
                    isEqualToString:@"Style Name"]) {
                if ([[tableColumn identifier]
                        isEqualToString:@"STYLE_CSOUND"]) {
                    NSInteger styleNumber =
                        [[KeyListCSound objectAtIndex:row] integerValue];
                    return [self StyleGetFriendlyName:@"csound"
                                          stylenumber:styleNumber];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"STYLE_PYTHON"]) {
                    NSInteger styleNumber =
                        [[KeyListPython objectAtIndex:row] integerValue];
                    return [self StyleGetFriendlyName:@"python"
                                          stylenumber:styleNumber];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"STYLE_LUA"]) {
                    NSInteger styleNumber =
                        [[KeyListLua objectAtIndex:row] integerValue];
                    return [self StyleGetFriendlyName:@"lua"
                                          stylenumber:styleNumber];
                }
            }

            if ([[[tableColumn headerCell] stringValue]
                    isEqualToString:@"Fore Color"]) {
                if ([[tableColumn identifier] isEqualToString:@"FORE_CSOUND"]) {
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetForeColor:@"csound"
                                                  stylenumber:styleNumber]]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"FORE_PYTHON"]) {
                    NSInteger styleNumber =
                        [[KeyListPython objectAtIndex:row] integerValue];
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetForeColor:@"python"
                                                  stylenumber:styleNumber]]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"FORE_LUA"]) {
                    NSInteger styleNumber =
                        [[KeyListLua objectAtIndex:row] integerValue];
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetForeColor:@"lua"
                                                  stylenumber:styleNumber]]];
                }

                return @""; //[self StyleGetForeColor:@"csound"
                            //stylenumber:styleNumber];
            }

            if ([[[tableColumn headerCell] stringValue]
                    isEqualToString:@"Back Color"]) {
                if ([[tableColumn identifier] isEqualToString:@"BACK_CSOUND"]) {
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetBackColor:@"csound"
                                                  stylenumber:styleNumber]]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"BACK_PYTHON"]) {
                    NSInteger styleNumber =
                        [[KeyListPython objectAtIndex:row] integerValue];
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetBackColor:@"python"
                                                  stylenumber:styleNumber]]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"BACK_LUA"]) {
                    NSInteger styleNumber =
                        [[KeyListLua objectAtIndex:row] integerValue];
                    ...
                    [[tableColumn dataCell]
                        setBackgroundColor:
                            [self HexToNSColor:
                                      [self StyleGetBackColor:@"lua"
                                                  stylenumber:styleNumber]]];
                }

                return @""; //[self StyleGetBackColor:@"csound"
                            //stylenumber:styleNumber];
            }

            if ([[[tableColumn headerCell] stringValue]
                    isEqualToString:@"Bold"]) {
                if ([[tableColumn identifier] isEqualToString:@"BOLD_CSOUND"]) {
                    NSInteger styleNumber =
                        [[KeyListCSound objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetBold:@"csound"
                                              stylenumber:styleNumber]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"BOLD_PYTHON"]) {
                    NSInteger styleNumber =
                        [[KeyListPython objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetBold:@"python"
                                              stylenumber:styleNumber]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"BOLD_LUA"]) {
                    NSInteger styleNumber =
                        [[KeyListLua objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetBold:@"lua"
                                              stylenumber:styleNumber]];
                }
            }

            if ([[[tableColumn headerCell] stringValue]
                    isEqualToString:@"Italic"]) {
                if ([[tableColumn identifier]
                        isEqualToString:@"ITALIC_CSOUND"]) {
                    NSInteger styleNumber =
                        [[KeyListCSound objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetItalic:@"csound"
                                                stylenumber:styleNumber]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"ITALIC_PYTHON"]) {
                    NSInteger styleNumber =
                        [[KeyListPython objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetItalic:@"python"
                                                stylenumber:styleNumber]];
                } else if ([[tableColumn identifier]
                               isEqualToString:@"ITALIC_LUA"]) {
                    NSInteger styleNumber =
                        [[KeyListLua objectAtIndex:row] integerValue];
                    return [NSNumber
                        numberWithBool:[self StyleGetItalic:@"lua"
                                                stylenumber:styleNumber]];
                }
            }
        }
...
- (IBAction)wxChangeBoldState:(id)sender {
    ...
    if ([sender isEqualTo:syntaxListCSound]) {
        if ([syntaxListCSound clickedColumn] != 3)
            return;
        language = @"csound";
    } else if ([sender isEqualTo:syntaxListPython]) {
        if ([syntaxListPython clickedColumn] != 3)
            return;
        language = @"python";
    } else if ([sender isEqualTo:syntaxListLua]) {
        if ([syntaxListLua clickedColumn] != 3)
            return;
        language = @"lua";
    } else
        return;
    ...
    NSString *Italic =
        ([self StyleGetItalic:language stylenumber:index] ? @"True" : @"False");
    NSString *Alpha =
        [NSString stringWithFormat:@"%d", [self StyleGetAlpha:language
                                                  stylenumber:index]];
    NSString *Eol =
        ([self StyleGetEolFilled:language stylenumber:index] ? @"True"
                                                             : @"False");
    NSString *FriendlyName = [self StyleGetFriendlyName:language
                                            stylenumber:index];

    NSString *value = [NSString
        stringWithFormat:@"%@,%@,%@,%@,%@,%@,%@", ForeColor, BackColor, Bold,
                         Italic, Alpha, Eol, FriendlyName];

    // NSLog(@"SYNTAX STRING STYLE: %@", value);

    [[wxDefaults valueForKey:valueForDefault]
        setValue:value
          forKey:[[NSNumber numberWithInteger:index] stringValue]];
}
...
- (IBAction)wxChangeItalicState:(id)sender {
    ...
    if ([sender isEqualTo:syntaxListCSound]) {
        if ([syntaxListCSound clickedColumn] != 4)
            return;
        language = @"csound";
    } else if ([sender isEqualTo:syntaxListPython]) {
        if ([syntaxListPython clickedColumn] != 4)
            return;
        language = @"python";
    } else if ([sender isEqualTo:syntaxListLua]) {
        if ([syntaxListLua clickedColumn] != 4)
            return;
        language = @"lua";
    } else
        return;
    ...
    NSString *BackColor = [self StyleGetBackColor:language stylenumber:index];
    NSString *Bold =
        ([self StyleGetBold:language stylenumber:index] ? @"True" : @"False");
...
- (IBAction)wxChangeColor:(id)sender {
    ...
    if ([syntaxList clickedRow] < 0)
        return;

    // NSLog(@"wxChangeColor - row: %d - Column %d", [syntaxList clickedRow],
    // [syntaxList clickedColumn]);
...
- (IBAction)wxBrowseFile:(id)sender {
    ...
    case 41:
    case 50:
        paths = NSSearchPathForDirectoriesInDomains(NSApplicationDirectory,
                                                    NSLocalDomainMask, YES);
    ...
    // SHEET WINDOW
    [openPanel
        beginSheetForDirectory:applicationDirectory
                          file:nil
                         types:nil //[NSArray
                                   //arrayWithObjects:@"txt",@"rtf",@"html",nil]
