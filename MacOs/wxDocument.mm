...
- (void)windowControllerDidLoadNib:(NSWindowController *)aController {
    ...
    // Set Window size
    NSInteger mWidth =
        [[wxDefaults valueForKey:@"DocumentWindowWidth"] integerValue];
    NSInteger mHeight =
        [[wxDefaults valueForKey:@"DocumentWindowHeight"] integerValue];
    if (mWidth < 690)
        mWidth = 690;
    if (mHeight < 350)
        mHeight = 350;
    ...
    if (textEditor == nil) {
        [wxMAIN
            ShowMessageError:@"wxDocument -> windowControllerDidLoadNib:"
                       error:[NSString
                                 stringWithFormat:
                                     @"%@\n%@", @"Critical error!",
                                     @"TextEditorView Initialization Failed!"]];
        return;
    }
    ...
    NSTableColumn *tableColumn =
        [[treeViewStructure tableColumns] objectAtIndex:0];
    ImageAndTextCell *imageAndTextCell =
        [[[ImageAndTextCell alloc] init] autorelease];

    [imageAndTextCell setEditable:NO];
    NSInteger fSize =
        11 + [[wxDefaults valueForKey:@"ExplorerFontSize"] integerValue];
    ...
    wxNode *newNode1 = nil, *newNode2 = nil, *newNode3 = nil, *newNode4 = nil,
           *newNode5 = nil, *newNode6 = nil;
    ...
    // Load Bookmarks ----------------------------------------------
    if ([self isSyntaxType]) // Load and Save bookmarks only on syntax styled
                             // documents (we skip others documents: txt, h)
    {
        @try {
            ...
            // if([[wxDefaults valueForKey:@"SaveBookmarks"] boolValue])
            {
                //@";[winxound_bookmarks"
                NSInteger ret =
                    [textEditor FindText:[NSString stringWithFormat:
                                                       @"%@[winxound_bookmarks",
                                                       languagecomment]
                          MatchWholeWord:true
                               MatchCase:true
                ...
                if (ret > -1) {
                    NSString *lineText = [textEditor
                        getTextOfLine:[textEditor
                    ...
                    NSArray *splittedNumbers =
                        [lineText componentsSeparatedByString:@","];

                    for (NSInteger index = 1; index < [splittedNumbers count];
                         index++) {
                        if (![[splittedNumbers objectAtIndex:index]
                                isEqualToString:@""])
                            [textEditor
                                InsertBookmarkAt:[[splittedNumbers
                                                     objectAtIndex:index]
                                                     integerValue]];
                    }
    ...
    if ([self fileURL] != nil) {
        if ([[NSFileManager defaultManager]
                isWritableFileAtPath:[[self fileURL] path]] == false) {
            // NSLog(@"NOT WRITABLE !!!!!!");
...
- (void)windowDidEndSheet:(NSNotification *)notification {
    ...
    [wxAUTOCOMP setDocumentOwner:self
                      withEditor:textEditor]; //??? Really necessary
}
...
- (void)windowWillClose:(NSNotification *)notification {
    if ([notification object] == mainWindow) //[self windowForSheet])
    {
        ...
        if ([[wxDefaults valueForKey:@"SaveBookmarks"] boolValue]) {
            ...
            // Save bookmarks only on syntax styled documents (CSound, Python,
            // Lua)
            if ([self isSyntaxType]) {
                if ([self fileURL] != nil && [self isDocumentEdited] == false) {
                    if ([[NSFileManager defaultManager]
                            isWritableFileAtPath:[[self fileURL] path]]) {
...
- (NSData *)dataOfType:(NSString *)typeName error:(NSError **)outError {
    ...
    // the given outError != NULL, ensure that you set *outError when returning
    // nil. You can also choose to override -fileWrapperOfType:error:,
    ...
    // choose to override -fileWrapperRepresentationOfType: or
...
- (BOOL)readFromData:(NSData *)data
    ...
    // *outError when returning NO. You can also choose to override
    ...
    _textContent = [[NSString alloc] initWithData:data
    ...
    if (_textContent == nil) {
        ...
        NSDictionary *options =
            [NSDictionary dictionaryWithObject:NSPlainTextDocumentType
...
//- (void)printShowingPrintPanel:(BOOL)flag
{
    ...
    [MyPrintInfo
        setLeftMargin:56.0]; // 72 = 25mm 56 = 20mm //45 = 15.9mm //28 = 10mm
    ...
    NSTextView *printView = [[NSTextView alloc]
        initWithFrame:
            ...
            NSMakeRect(0.0, 0.0, [MyPrintInfo paperSize].width,
                       [MyPrintInfo paperSize].height)];
    //[MyPrintInfo imageablePageBounds]];
    ...
    // NSRange printViewRange = NSMakeRange(0, [[printView textStorage]
    // length]);

    //[printView replaceCharactersInRange: printViewRange
    //							withRTF:[textView RTFFromRange: textViewRange]];

    op = [NSPrintOperation printOperationWithView:printView
    ...
    [printPanel
        setOptions:[printPanel options] | NSPrintPanelShowsPageSetupAccessory];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
    ...
    // Disable Comments if the document is not Syntax Type document (txt or
    // others)
    ...
    if ([menuItem action] == @selector(CompileWithAdditionalFlagsClicked:) ||
        [menuItem action] == @selector
            (CompileExternalWithAdditionalFlagsClicked:)) {
        return ([[self fileType] isEqualToString:@"CSound Files"] ||
                [[self fileType] isEqualToString:@"CSound Orc Files"] ||
                [[self fileType] isEqualToString:@"CSound Sco Files"]);
    }
...
- (BOOL)revertToContentsOfURL:(NSURL *)absoluteURL
    ...
    BOOL success = [super revertToContentsOfURL:absoluteURL
                                         ofType:typeName
                                          error:outError];
...
- (BOOL)saveToURL:(NSURL *)absoluteURL
    ...
    forSaveOperation:(NSSaveOperationType)saveOperation
               error:(NSError **)outError {
    BOOL ret = [super saveToURL:absoluteURL
                         ofType:typeName
...
- (void)lookForOrcScoFiles {
    // NSLog(@"LOOKFOR:%@", [[[self fileURL] path]
    ...
    NSArray *onlyOrcSco = [dirContents
        filteredArrayUsingPredicate:[NSPredicate
                                        predicateWithFormat:predicate]];

    if (onlyOrcSco == nil)
        return;
    if ([onlyOrcSco count] < 1)
        return;

    // Append files to the combobox
    for (NSString *file in onlyOrcSco) {
        [linkBoxComboBox
            addItemWithObjectValue:[NSString
                                       stringWithFormat:@"%@/%@", dir, file]];
    }
    ...
    for (NSInteger index = 0; index < [linkBoxComboBox numberOfItems];
        ...
        if ([value rangeOfString:[[self fileURL] path]].location !=
                NSNotFound ||
    ...
    @try {
        if (returnCode == NSOKButton) {
            ...
            for (NSURL *url in filesToOpen) {
                NSInteger index =
                    [linkBoxComboBox indexOfItemWithObjectValue:[url path]];
            ...
            if (index != -1)
                ...
                [linkBoxComboBox
                    selectItemAtIndex:[linkBoxComboBox numberOfItems] - 1];
        }
...
- (IBAction)wxLinkBoxOpenLink:(id)sender {
    if ([self IsOrcScoFile]) {
        if ([[NSFileManager defaultManager]
                fileExistsAtPath:[linkBoxComboBox stringValue]])
            [wxMAIN
                wxOpenOrcScoDocumentWithFilename:[linkBoxComboBox stringValue]];
    }
}
//----------------------------------------------------------------------------------------------------------
// ORC/SCO METHODS
...
//----------------------------------------------------------------------------------------------------------
// WinXound use a timer to start the FindStructure method: the timer is fired
// after about 1.2 seconds (based on SCI_MODIFIED) wxGlobal.h define the
// interval (#define FIND_STRUCTURE_INTERVAL 6)
- (void)startTimer {
...
- (void)timerFireMethod:(NSTimer *)theTimer {
    ...
    if (timerCounter >= FIND_STRUCTURE_INTERVAL) {
        timerCounter = 0;
        if (isChanged == true &&
            [self isSyntaxType]) // Skip findStructure for Non-Syntax documents
        {
            [NSThread detachNewThreadSelector:@selector(findStructure:)
                                     toTarget:self
                                   withObject:[textEditor getText]];
            isChanged = false;
        }
    }
}
...
// has changed. Now would be a good time to update any container UI elements
// that depend on document or view state.
- (void)SCI_UPDATEUI_Notification:(NSNotification *)notification {
    ...
    if ([[wxDefaults valueForKey:@"EditorShowMatchingBracket"] boolValue] ==
        true)
    ...
    @try {
        ...
        NSLog(@"wxDocument -> SCI_UPDATEUI_Notification Error: %@ - %@",
              [e name], [e reason]);
    }
...
- (void)setIntelliTipTextForOpcode:(NSString *)opcode {
    ...
    if ([description length] > 0) {
        [intelliTipTitle
            setStringValue:[NSString stringWithFormat:@"[%@] - %@", opcode,
                                                      [items objectAtIndex:1]]];
        [intelliTipParams
            setStringValue:[NSString stringWithFormat:@"Parameters: %@",
...
- (void)checkForBracket {
    @try {
        ...
        if ([[textEditor getCharAt:(caretpos - 1)] isEqualToString:@")"]) {
            ...
            NSInteger pos1 = [textEditor BraceMatch:(caretpos - 1)
                                         maxReStyle:0];
            if (pos1 < 0) {
        ...
        NSLog(@"wxDocument -> checkForBracket Error: %@ - %@", [e name],
              [e reason]);
    }
}

// SCN_MODIFIED
// This notification is sent when the text or styling of the document changes or
// is about to change. You can set a mask for the notifications that are sent to
// the container with SCI_SETMODEVENTMASK. The notification structure contains
// information about what changed, how the change occurred and whether this
// changed the number of lines in the document. No modifications may be
// performed while in a SCN_MODIFIED event.
...
- (void)CheckUndoRedo {
    if ([[textEditor getPrimaryView] getGeneralProperty:SCI_GETMODIFY
                                              parameter:0]) {
...
- (void)SCI_MOD_CONTAINER_Notification:(NSNotification *)notification {
    ...
    NSInteger token =
        [[[notification userInfo] objectForKey:@"token"] integerValue];
...
// SCN_ZOOM
// This notification is generated when the user zooms the display using the
// keyboard or the SCI_SETZOOM method is called. This notification can be used
// to recalculate positions, such as the width of the line number margin to
// maintain sizes in terms of characters rather than pixels.
- (void)SCI_ZOOM_Notification:(NSNotification *)notification {
    @try {
        // NSLog(@"ZOOM NOTIFICATION");
        NSInteger fontSize =
            [[textEditor getPrimaryView] getGeneralProperty:SCI_STYLEGETSIZE
                                                  parameter:STYLE_DEFAULT];
        NSInteger percent =
            ((fontSize + [textEditor getZoomForView1]) * 100 / fontSize);

        if ([textEditor getIsSplitted]) {
            NSInteger fontSize2 = [[textEditor getSecondaryView]
                getGeneralProperty:SCI_STYLEGETSIZE
                         parameter:STYLE_DEFAULT];
            NSInteger percent2 =
                ((fontSize2 + [textEditor getZoomForView2]) * 100 / fontSize2);
            [textFieldFontSize
                setStringValue:[NSString stringWithFormat:@"Zoom: %d%%/%d%%",
                                                          percent, percent2]];
        } else {
            [textFieldFontSize
                setStringValue:[NSString
                                   stringWithFormat:@"Zoom: %d%%", percent]];
        }

    } @catch (NSException *e) {
        NSLog(@"wxDocument -> SCI_ZOOM_Notification Error: %@ - %@", [e name],
              [e reason]);
    }
}
...
- (void)SCI_LEFTMOUSEHYPERLINKS_Notification:(NSNotification *)notification {
    ...
    @try {
        ...
        if (([textEditor GetStyleAt:[textEditor getCaretPosition]] == 6 &&
             [_Language isEqualToString:@"csound"]) ||
            ([textEditor GetStyleAt:[textEditor getCaretPosition]] == 6 &&
             [_Language isEqualToString:@"lua"]) ||
            ([textEditor GetStyleAt:[textEditor getCaretPosition]] == 3 &&
             [_Language isEqualToString:@"python"]) ||
            ([textEditor GetStyleAt:[textEditor getCaretPosition]] == 4 &&
             [_Language isEqualToString:@"python"])) // single quoted string
        {
            NSString *stringChar = nil;
            NSString *mString = nil;
            NSInteger mStart = 0;
            NSInteger mEnd = 0;
            NSInteger mCaretPosition = [textEditor
                getCaretPosition]; // tEditor.textEditor.GetCaretPosition();
            NSInteger mCurrentLineNumber = [textEditor
                getCurrentLineNumber]; // tEditor.textEditor.GetCurrentLineNumber();
            NSInteger mLineStart =
                [textEditor getPositionFromLineNumber:mCurrentLineNumber];
            NSInteger mLineEnd = [textEditor
                GetLineEndPosition:[textEditor getCurrentLineNumber]];

            if ([textEditor GetStyleAt:[textEditor getCaretPosition]] == 4 &&
                [_Language isEqualToString:@"python"]) // single quoted string
            {
                stringChar = @"'";
            } else
            ...
            if ((mEnd - mStart) > 0 && mStart >= 0) {
                // mString = tEditor.textEditor.GetText().Substring(mStart, mEnd
                // - mStart); mString = [[textEditor getText]
                // substringWithRange:NSMakeRange(mStart, mEnd-mStart)];
                mString = [textEditor
                    getTextRange:NSMakeRange(mStart, mEnd - mStart)];
                [self OpenHyperLinks:mString];
            }
        }

    } @catch (NSException *e) {
        [wxMAIN ShowMessageError:
                    @"wxDocument -> SCI_LEFTMOUSEHYPERLINKS_Notificaiton:"
                           error:[NSString stringWithFormat:@"%@\n%@", [e name],
                                                            [e reason]]];
    }
}
...
- (void)OpenHyperLinks:(NSString *)mString {
    ...
    // if (File.Exists(Path.GetDirectoryName(ActiveEditor.FileName) + "\\" +
    // mString))
                ...
                [NSString
                    stringWithFormat:@"%@/%@",
                                     [[self fileName]
                                         stringByDeletingLastPathComponent],
                                     mString]]) {
        // mString = Path.GetDirectoryName(ActiveEditor.FileName) + "\\" +
        // mString;
        mString =
    ...
    // Search for INCDIR
    // else if (File.Exists(wxGlobal.Settings.Directory.INCDIR + "\\" +
    // mString))
    else if ([fileManager
                 fileExistsAtPath:
                     [NSString stringWithFormat:@"%@/%@",
                                                [wxDefaults
                                                    valueForKey:@"INCDIRPath"],
                                                mString]]) {
        // mString = wxGlobal.Settings.Directory.INCDIR + "\\" + mString;
        mString = [NSString
            stringWithFormat:@"%@/%@", [wxDefaults valueForKey:@"INCDIRPath"],
                             mString];
    }
    // Search for SFDIR
                 ...
                 fileExistsAtPath:
                     [NSString
                         stringWithFormat:@"%@/%@",
                                          ...
                                          mString]]) {
        mString = [NSString
            stringWithFormat:@"%@/%@", [wxDefaults valueForKey:@"SFDIRPath"],
                             mString];
    }
    // Search for SSDIR
                 ...
                 fileExistsAtPath:
                     [NSString
                         stringWithFormat:@"%@/%@",
                                          ...
                                          mString]]) {
        mString = [NSString
            stringWithFormat:@"%@/%@", [wxDefaults valueForKey:@"SSDIRPath"],
                             mString];
    }
    // Search for SADIR
                 ...
                 fileExistsAtPath:
                     [NSString
                         stringWithFormat:@"%@/%@",
                                          ...
                                          mString]]) {
        mString = [NSString
            stringWithFormat:@"%@/%@", [wxDefaults valueForKey:@"SADIRPath"],
                             mString];
    }
    // Search for MFDIR
                 ...
                 fileExistsAtPath:
                     [NSString
                         stringWithFormat:@"%@/%@",
                                          ...
                                          mString]]) {
        mString = [NSString
            stringWithFormat:@"%@/%@", [wxDefaults valueForKey:@"MFDIRPath"],
                             mString];
    }
    ...
    if (![fileManager fileExistsAtPath:mString])
        return;
    ...
    if ([[mString lowercaseString] hasSuffix:@".wav"] ||
        ...
        if ([wxDefaults valueForKey:@"WaveEditorPath"] != nil &&
            ...
            if ([fileManager
                    fileExistsAtPath:[wxDefaults
                                         valueForKey:@"WaveEditorPath"]]) {
                //				System.Diagnostics.Process.Start(
                //												 wxGlobal.Settings.Directory.WaveEditor,
                //												 "\"" + mString
                //+ "\"");
                //[[NSWorkspace sharedWorkspace] launchApplication:
                // [NSString stringWithFormat:@"%@ @"/Applications/QuickTime
                // Player.app"];

                //-a /Applications/QuteCsound.app --args
                //"/Users/teto/Desktop/113.csd"
...
- (void)SCI_RIGHTMOUSE_Notification:(NSNotification *)notification {
    ...
    [itemDefinition
        setTitle:[NSString stringWithFormat:@"Go to definition of ..."]];
    [itemReference
        setTitle:[NSString stringWithFormat:@"Go to reference of ..."]];
    ...
    if ([self isSyntaxType]) {
        if ([wxMAIN getOpcodeValue:[textEditor getCurrentWord]] != nil) {
            [itemHelp
                setTitle:[NSString
                             stringWithFormat:@"\"%@\" Help",
                                              [textEditor getCurrentWord]]];
            ...
            [itemDefinition
                setTitle:[NSString
                             stringWithFormat:@"Go to definition of %@",
                                              [textEditor getCurrentWord]]];
            [itemReference
                setTitle:[NSString
                             stringWithFormat:@"Go to reference of %@",
                                              [textEditor getCurrentWord]]];
        }

        // if([textEditor GetStyleAt:[textEditor getCaretPosition]] == 6)
        NSLog(@"textstyle: %d",
              [textEditor GetStyleAt:[textEditor getCaretPosition]]);
        if (([textEditor GetStyleAt:[textEditor getCaretPosition]] == 6 &&
             [_Language isEqualToString:@"csound"]) ||
            ([textEditor GetStyleAt:[textEditor getCaretPosition]] == 6 &&
             [_Language isEqualToString:@"lua"]) ||
...
- (void)SCI_LEFTMOUSE_QUOTES_Notification:(NSNotification *)notification {
    ...
    NSInteger curPos =
        [[[notification userInfo] objectForKey:@"position"] integerValue];

    if (([textEditor GetStyleAt:curPos] == 6 &&
         [_Language isEqualToString:@"csound"]) ||
        ([textEditor GetStyleAt:curPos] == 6 &&
         [_Language isEqualToString:@"lua"]) ||
        ([textEditor GetStyleAt:curPos] == 3 &&
         [_Language isEqualToString:@"python"]) ||
        ([textEditor GetStyleAt:curPos] == 4 &&
         [_Language isEqualToString:@"python"])) {

        NSString *stringChar = nil;

        if ([textEditor GetStyleAt:curPos] == 4 &&
            [_Language isEqualToString:@"python"]) // single quoted string
        {
            stringChar = @"'";
        } else
...
- (void)splitView:(NSSplitView *)splitView
    ...
    if (splitView == mainSplitWindow) {
        ...
        leftFrame.size.width =
            (leftFrame.size.width < SplitMargins ? SplitMargins
                                                 : leftFrame.size.width);
        ...
        if (rightFrame.size.width < SplitMargins)
            rightFrame.size.width = SplitMargins;
        ...
        // upFrame.size.height = (upFrame.size.height < SplitMargins ?
        // SplitMargins : upFrame.size.height); upFrame.size.height =
        // newFrame.size.height - downFrame.size.height - dividerThickness;

        if (upFrame.size.height < SplitMargins)
            upFrame.size.height = SplitMargins;
        ...
        // downFrame.size.height = (downFrame.size.height <= SplitMargins ?
        // SplitMargins : downFrame.size.height);
        downFrame.size.height = SplitMargins;
...
- (IBAction)wxSplitHorizontal:(id)sender {
    ...
    if ([textEditor getIsSplitted]) {
        [textEditor
            setFirstVisibleLineAtView:[textEditor getFirstVisibleLineAtView:1]
                                 view:2];
    }
    [self SCI_ZOOM_Notification:nil];
}
...
- (IBAction)wxInsertRemoveBookmark:(id)sender {
    ...
    @try {
        ...
        do {
            ...
            // ListBoxBookmarks.Items.Add(String.Format("{0:G}", mIndex) + ". "
            // + mText);
...
- (IBAction)wxTreeViewMouseClick:(id)sender {
    @try {
        wxNode *item =
            [treeViewStructure itemAtRow:[treeViewStructure selectedRow]];
        ...
        if ([item.name isEqualToString:@"Bookmarks"]) {
            // DO NOTHING
                     ...
                     isEqualToString:@"Bookmarks"]) {
            // NSLog(@"INSIDE BOOKMARKS");
            NSInteger CurLine = 0;
            NSInteger BookLine = 0;
            NSInteger mIndex =
                [treeViewStructure selectedRow] -
                [treeViewStructure
                    rowForItem:[treeViewStructure parentForItem:item]] -
                1;

            for (NSInteger Ciclo = 0; Ciclo < mIndex + 1; Ciclo++) {
                // BookLine = textEditor.GetFocusedEditor.MarkerNext(CurLine,
                // 1);
            ...
            do {
                ...
                if (ret > -1) {
                    NSString *textOfLine = [[textEditor
                        getTextOfLine:[textEditor
                                          getLineNumberFromPosition:ret]]
                    ...
                    // NSLog(@"Scintilla lineLength: %d - Node line length: %d",
                    // length, [item.extendedname length]);
                    ...
                    break;
                     ...
                     isEqualToString:@"<CsScore>"] &&
                 [item.name hasPrefix:@"s"]) {
            // if([item.name hasPrefix:@"s"])
            {
                NSInteger cycles =
                    [[item.name substringFromIndex:2] integerValue];
                NSInteger mStart = 0;
                ...
                if (ret > -1) {
                    [textEditor
                        GoToLine:[textEditor getLineNumberFromPosition:ret]];
                }
            }
        }
            ...
            // BOOL backward = false;
            if ([[[treeViewStructure parentForItem:item] name]
                    isEqualToString:@"<CsScore>"]) {
                // Search backward
                // backward = true;
        ...
        if (ret > -1) {
            [textEditor SelectLine:[textEditor getCurrentLineNumber]
                SetAsFirstVisibleLine:true];
            //[textEditor setFirstVisibleLine:[textEditor getCurrentLineNumber]
            //-1];
        }
    ...
    if (panelGoToLine != nil) {
        [NSApp beginSheet:panelGoToLine
            modalForWindow:[self windowForSheet]
             modalDelegate:self
            didEndSelector:
                nil //@selector(sheetDidFinish:returnCode:contextInfo:)
               contextInfo:nil];
...
- (IBAction)wxShowCode:(id)sender {
    // SplitMargins
    [compilerSplitWindow
             setPosition:[compilerSplitWindow frame].size.height - SplitMargins
        ofDividerAtIndex:0];
}
...
- (IBAction)wxCommentLine:(id)sender {
    @try {
        ...
        NSInteger mLineStart = [textEditor
            getLineNumberFromPosition:[textEditor getSelectionStart]];

        // GetFocusedEditor.LineFromPosition(GetFocusedEditor.GetSelectionEnd());
        NSInteger mLineEnd =
            [textEditor getLineNumberFromPosition:[textEditor getSelectionEnd]];
...
- (IBAction)wxZoomFontSize:(id)sender {
    @try {
        ...
        case 0:
            if (zoom > -10)
                [textEditor setZoom:[textEditor getZoom] - 1];
            break;
        case 1:
            if (zoom < 20)
                [textEditor setZoom:[textEditor getZoom] + 1];
...
- (IBAction)wxGotoDefinitionOf:(id)sender {
    if ([[textEditor getCurrentWord] length] < 1)
        return;
    ...
    @try {
        ...
        if ([wxMAIN getOpcodeValue:Definition] == nil) {
            ////MACRO SEARCH
            if ([Definition hasPrefix:@"g"] || [Definition hasPrefix:@"$"]) {
                if ([Definition hasPrefix:@"$"]) {
                    Definition = [Definition
                        substringFromIndex:1]; // Definition.TrimStart('$');
                }
                ...
                if (mPos > -1)
                    [_CursorPosition StoreCursorPos:mPos];
                return;
            ...
            ////LOCAL_TYPE SEARCH: search inside "instr" -> "endin"
                ...
                NSInteger curPos = [textEditor
                    getCaretPosition]; // ActiveEditor.textEditor.GetCaretPosition();
                ...
                if (mFindPos > -1)
                    [_CursorPosition StoreCursorPos:mFindPos];
            }
        }
...
- (IBAction)wxGotoReferenceOf:(id)sender {
    ...
    @try {
        ...
        if ([wxMAIN getOpcodeValue:Definition] == nil) {
            ...
            // if
            // (ActiveEditor.textEditor.GetTextLine(ActiveEditor.textEditor.GetCurrentLineNumber()).Contains("#define"))
            if ([[textEditor getTextOfLine:[textEditor getCurrentLineNumber]]
                    rangeOfString:@"#define"]
                    .location != NSNotFound) {
                NSString *NewDefinition =
...
- (IBAction)wxFormatCodeAll:(id)sender {
    ...
    // NSLog(@"Format All");
    [[wxCodeFormatter sharedInstance] formatCode:textEditor start:-1 end:-1];
}
...
- (IBAction)wxShowDocInfo:(id)sender {
    if (panelDocInfo != nil) {
        ...
        [labelInfoTotalLines
            setStringValue:[NSString
                               stringWithFormat:@"%d",
                                                [textEditor getLinesCount]]];
        [labelInfoTotalChars
            setStringValue:[NSString
                               stringWithFormat:@"%d",
                                                [textEditor getTextLength]]];
...
- (IBAction)wxShowCodeRepositoryWindow:(id)sender {
    ...
    // Set window position
    NSPoint p =
        NSMakePoint([[self windowForSheet] frame].origin.x +
                        [[self windowForSheet] frame].size.width / 2 - 305,
                    [[self windowForSheet] frame].origin.y +
                        [downView frame].size.height - 285);
...
- (IBAction)wxShowCSoundRepositoryWindow:(id)sender {
    ...
    NSPoint p =
        NSMakePoint([[self windowForSheet] frame].origin.x +
                        [[self windowForSheet] frame].size.width / 2 - 290,
                    [[self windowForSheet] frame].origin.y +
                        [downView frame].size.height - 308);
...
- (IBAction)wxShowAutocompleteWindow:(id)sender {
    ...
    NSInteger curLine = [textEditor
        getCurrentLineNumber]; // ActiveEditor.textEditor.GetCurrentLineNumber();
    NSInteger firstVisibleLine = [textEditor
        getFirstVisibleLine]; // ActiveEditor.textEditor.GetFirstVisibleLine();
    NSInteger lastVisibleLine =
        firstVisibleLine +
        [textEditor
            getLinesOnScreen]; // ActiveEditor.textEditor.LinesOnScreen();
    ...
    NSPoint p = NSMakePoint(
        [[textEditor getFocusedEditor] getGeneralProperty:SCI_POINTXFROMPOSITION
                                                parameter:0
                                                    extra:curPos] +
            offsetX,
        [[textEditor getFocusedEditor] getGeneralProperty:SCI_POINTYFROMPOSITION
                                                parameter:0
                                                    extra:nextPos] +
            offsetY);
    ...
    if ([textEditor getIsSplitted] == false)
        ...
        if ([textEditor getFocusedEditor] == [textEditor getPrimaryView]) {
            ...
            p.y = [codeView frame].size.height -
                  [[textEditor getSecondaryView] frame].size.height -
                  -[textEditor dividerThickness] - [NSScroller scrollerWidth] -
                  p.y - 12;
    }
    ...
    // NSPoint result = [[self windowForSheet]convert convertPoint: p fromView:
    // nil];
...
- (NSString *)saveFileBeforeCompile {
    ...
    // Check if WinXound can compile the file (only csound, python or lua
    ...
    if ([self fileURL] == nil || ![[NSFileManager defaultManager]
                                     fileExistsAtPath:[[self fileURL] path]]) {
        // Old behaviour:
        ...
        if (paths != nil) {
            if ([paths count] > 0) {
                NSString *winxoundPath = [[NSString
                    stringWithFormat:@"%@/WinXound/Temp",
                                     [paths objectAtIndex:0]] retain];
                ...
                tempFilename =
                    [NSString stringWithFormat:@"%@/WinXound_Untitled%@",
                                               winxoundPath, tempExtension];
...
- (void)Compile:(NSString *)additionalParams External:(bool)external {
    ...
    if ([TextEditorFileName isEqualToString:@""])
        return;
    ...
    if ([[TextEditorFileName lowercaseString] hasSuffix:@".lua"]) {
        if ([[wxDefaults valueForKey:@"LuaConsolePath"] isEqualToString:@""]) {
            [wxMAIN ShowMessage:@"Lua Compiler not found!"
                informativeText:@"Please specify a valid path in your WinXound "
                                @"Preferences (Directories tab)."
                  defaultButton:@"OK"
                alternateButton:nil
                    otherButton:nil];
            return;
    ...
    // PYTHON COMPILER
        ...
        if ([[wxDefaults valueForKey:@"PythonConsolePath"]
                isEqualToString:@""]) {
    ...
    // CSOUND COMPILER
        ...
        if ([[wxDefaults valueForKey:@"CSoundConsolePath"]
                isEqualToString:@""]) {
        ...
        parameters = [NSString
            stringWithFormat:@"%@ %@", additionalParams, [self getCSoundFlags]];
...
- (void)UpdateAdditionalFlagsPopupButtonList {
    ...
    NSString* imageName = [[NSBundle mainBundle] pathForResource:@"compile_16"
    ofType:@"png"]; NSImage* imageObj = [[[NSImage alloc]
    initWithContentsOfFile:imageName] autorelease]; [AdditionalFlagsPopup
    addItemWithTitle:@""];
    [[AdditionalFlagsPopup itemAtIndex:0] setImage:imageObj];
    */

    // Clear all items except the first one (it contains the Compile image)
    for (NSInteger index = [AdditionalFlagsPopup numberOfItems] - 1; index > 0;
         index--) {
        [AdditionalFlagsPopup removeItemAtIndex:index];
    }

    // Set the AdditionalFlags menu's
    NSArray *lines = [[wxDefaults valueForKey:@"CSoundAdditionalFlags"]
        componentsSeparatedByString:@"\n"];

    // Add items and connect to the documents method
    // (CompileWithAdditionFlagsClicked ... look at wxDocument!)
...
- (NSString *)GetAdditionalFlagsOptions:(NSString *)options {
    @try {
        ...
        [wxMAIN ShowMessage:@"Additional Flags: Invalid syntax!"
            informativeText:
                @"Please specify a valid command ([Description]: Flags) in "
                @"your WinXound Preferences (Compiler tab)."
              defaultButton:@"OK"
            alternateButton:nil
                otherButton:nil];
    }
...
- (NSString *)getCSoundFlags {
    ...
    // If --displays flag is present we must show csound windows and suppress
    // the
    ...
    @try {
        ...
        if (findStart > -1 && findEnd > -1) {
            ...
            for (NSInteger i = linestart; i < lineend; i++) {
                ...
                if ([text rangeOfString:@";"].location != NSNotFound) {
                    ...
                    text = [text
                        substringToIndex:[text rangeOfString:@";"].location];
                    // if (text.Contains("--displays") /*||
                    //	text.Contains("--nodisplays") ||
                    //	text.Contains("-d")*/)
                    if ([text rangeOfString:@"--displays"].location !=
                        NSNotFound) {
                        // if (!text.Contains("--asciidisplay") &&
                        //	!text.Contains("-g") &&
                        //	!text.Contains("--postscriptdisplay") &&
                        //	!text.Contains("-G"))
                        if ([text rangeOfString:@"--asciidisplay"].location ==
                                NSNotFound &&
                            [text rangeOfString:@"-g"].location == NSNotFound &&
                            [text rangeOfString:@"--postscriptdisplay"]
                                    .location == NSNotFound &&
                            [text rangeOfString:@"-G"].location == NSNotFound) {
                            // Found --displays option so:
                            // temporarily remove all 'display(s)' settings from
                            // the default winxound settings and leave the
                            // others
                            ...
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:
                                    @"--displays"
                                                          withString:@""];
                            // defaultflags =
                            // defaultflags.Replace("--nodisplays", "");
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:
                                    @"--nodisplays"
                                                          withString:@""];
                            ...
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:@"-d "
                                                          withString:@" "];
                            // defaultflags =
                            // defaultflags.Replace("--asciidisplay", "");
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:
                                    @"--asciidisplay"
                                                          withString:@""];
                            ...
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:@"-g "
                                                          withString:@" "];
                            // defaultflags =
                            // defaultflags.Replace("--postscriptdisplay", "");
                            defaultflags = [defaultflags
                                stringByReplacingOccurrencesOfString:
                                    @"--postscriptdisplay"
                                                          withString:@""];
                            ...
                            // NSLog(@"CheckDisplaysFlag = true - parameters:
                            // %@", defaultflags);
        ...
        NSLog(@"wxDocument -> getCSoundFlags Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSString *)CheckForAdditionalFlags:(NSString *)additionalParams {
    ...
    // else if (additionalParams.Contains("?"))
        ...
        int result = [spanel
            runModalForDirectory:nil
                            file:[[[[self fileURL] path] lastPathComponent]
                                     stringByDeletingPathExtension]];
...
- (IBAction)wxCallExternalGui:(id)sender {
    if ([[self saveFileBeforeCompile] isEqualToString:@""])
        return;
    ...
    if ([[[self fileName] lowercaseString] hasSuffix:@".lua"]) {
        if ([[wxDefaults valueForKey:@"LuaExternalGuiPath"]
                isEqualToString:@""]) {
    ...
    // PYTHON GUI
        ...
        if ([[wxDefaults valueForKey:@"PythonExternalGuiPath"]
                isEqualToString:@""]) {
    ...
    // CSOUND GUI
        ...
        if ([[wxDefaults valueForKey:@"CSoundExternalGuiPath"]
                isEqualToString:@""]) {
    ...
    //[task setCurrentDirectoryPath:@"/Users/Teto/Desktop"];
    [task setArguments:[NSArray
                           arrayWithObjects:@"-a", compiler, @"--args",
...
- (void)configureEditor {
    ...
    if ([[self fileType] isEqualToString:@"Python Files"]) {
        ...
        if ([[wxDefaults valueForKey:@"EditorUseMixedPython"] boolValue] ==
            true) {
    ...
    [textEditor
        setTextEditorFont:[NSFont
                              fontWithName:[wxDefaults
                                               valueForKey:@"EditorFontName"]
                                      size:[[wxDefaults
                                               valueForKey:@"EditorFontSize"]
                                               floatValue]]];
    [textEditor
        setShowLineNumbers:[[wxDefaults valueForKey:@"EditorShowLineNumbers"]
                               boolValue]];
    [self showExplorer:[[wxDefaults valueForKey:@"EditorShowExplorer"]
                           boolValue]];
    [self showOnlineHelp:[[wxDefaults valueForKey:@"EditorShowIntellitip"]
                             boolValue]];
    [self
        showToolbar:[[wxDefaults valueForKey:@"EditorShowToolbar"] boolValue]];
    [textEditor setShowSpaces:false];
    [textEditor setShowEOLMarker:false];
    [textEditor
        setShowMatchingBracket:[[wxDefaults
                                   valueForKey:@"EditorShowMatchingBracket"]
                                   boolValue]];
    [textEditor setShowVerticalRuler:[[wxDefaults
                                         valueForKey:@"EditorShowVerticalRuler"]
                                         boolValue]];
    [textEditor
        setMarkCaretLine:[[wxDefaults valueForKey:@"EditorMarkCaretLine"]
                             boolValue]];
    [textEditor setTabIndent:[[wxDefaults valueForKey:@"EditorTabIndent"]
                                 integerValue]];
    ...
    [textEditor
        setScrollWidth:[[textEditor getPrimaryView] getTextRectangleWidth]];
...
- (void)configureEditorForCSound:(NSDictionary *)opcodes {
    ...
    if (opcodes != nil) {
        ...
        for (NSString *aKey in opcodes) {
            if ([aKey isEqualToString:@"instr"] ||
                [aKey isEqualToString:@"endin"])
                continue;
        ...
        NSString *CabbageWidgets =
            @"form rslider hslider vslider "
             "button checkbox combobox groupbox keyboard";
    ...
    [textEditor setWordChars:@"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUV"
                             @"WXYZ0123456789.$_"];
...
- (void)configureEditorForLua {
    ...
    // Add Lua 4:
    [keywords2 appendString:@"_ALERT _ERRORMESSAGE _INPUT _PROMPT _OUTPUT "
                             "_STDERR _STDIN _STDOUT call dostring foreach "
                             "foreachi getn globals newtype"
                             "sort tinsert tremove "];

    [keywords3 appendString:@"abs acos asin atan atan2 ceil cos deg exp "
    ...
    // Add Lua 5:
    [keywords2
        appendString:@"_G getfenv getmetatable ipairs loadlib next pairs pcall "
                      "rawequal setfenv setmetatable xpcall "
                      "string table math coroutine io os debug "
                      "load module select "];

    [keywords3
        appendString:@"string.byte string.char string.dump string.find "
    ...
    [keywords4
        appendString:@"coroutine.create coroutine.resume coroutine.status "
    ...
    [textEditor setKeyWords:1
                keyWordList:[NSString stringWithString:keywords2]];
    [textEditor setKeyWords:2
                keyWordList:[NSString stringWithString:keywords3]];
    [textEditor setKeyWords:3
                keyWordList:[NSString stringWithString:keywords4]];
...
- (void)SciEditSetFontsAndStyles {
    ...
    for (NSInteger mIndex = 0; mIndex < 34; mIndex++) {
        ...
        [textEditor StyleSetBold:mIndex
                            bold:[[wxMAIN getSettings] StyleGetBold:_Language
                                                        stylenumber:mIndex]];

        [textEditor
            StyleSetItalic:mIndex
                    italic:[[wxMAIN getSettings] StyleGetItalic:_Language
                                                    stylenumber:mIndex]];
    }

    // DEFAULT STYLE "34" STYLE_BRACELIGHT
    [textEditor
        StyleSetFore:STYLE_BRACELIGHT
           htmlcolor:[[wxMAIN getSettings]
                         StyleGetForeColor:_Language
                               stylenumber:32]]; // Same as TextForeColor
    [textEditor
        StyleSetBack:STYLE_BRACELIGHT
           htmlcolor:[[wxMAIN getSettings]
                         StyleGetBackColor:_Language
                               stylenumber:256]]; // TextSelectionBackColor
    ...
    [textEditor MarkerSetAlpha:0
                      intcolor:[[wxMAIN getSettings] StyleGetAlpha:_Language
                                                       stylenumber:257]];

    // VERTICAL RULER (style 258)
    [textEditor setEdgeColor:[[wxMAIN getSettings] StyleGetForeColor:_Language
                                                         stylenumber:258]];

    // CARET LINE MARKER (style 259)
    [textEditor
        setCaretLineBack:[[wxMAIN getSettings] StyleGetForeColor:_Language
                                                     stylenumber:259]];

    // CARET COLOR (Same as Text Fore Color)
    [textEditor setCaretFore:[[wxMAIN getSettings] StyleGetForeColor:_Language
                                                         stylenumber:32]];
...
- (void)configureCompiler {
    ...
    [compilerOutput
        setFont:[NSFont
                    fontWithName:[wxDefaults valueForKey:@"CompilerFontName"]
                            size:[[wxDefaults valueForKey:@"CompilerFontSize"]
                                     floatValue]]];
    ...
    [self setUseWinXoundFlags:[[wxDefaults valueForKey:@"UseWinxoundFlags"]
                                  boolValue]];
}
...
- (void)setTextContent:(NSString *)text {
    [[textEditor getPrimaryView] setGeneralProperty:SCI_BEGINUNDOACTION
                                          parameter:0
                                              value:0];
    [textEditor setText:text];
    [textEditor ConvertEOL:[textEditor GetEOLMode]];
    [[textEditor getPrimaryView] setGeneralProperty:SCI_ENDUNDOACTION
                                          parameter:0
                                              value:0];
}
...
- (void)checkLineEndings {
    ...
    NSString *s =
        [[textEditor getText] stringByReplacingOccurrencesOfString:@"\r\n"
                                                        withString:@""];
    ...
    if (crlfOccurrences != [textEditor getLinesCount] - 1) {
        ...
        lfOccurrences =
            [s length] - [[s stringByReplacingOccurrencesOfString:@"\n"
                                                       withString:@""] length];
    ...
    // CRLF
    if (crlfOccurrences > crOccurrences && crlfOccurrences > lfOccurrences) {
    ...
    // CR
    else if (crOccurrences > crlfOccurrences && crOccurrences > lfOccurrences) {
        // Set EOL mode to CR
        [textEditor setEolMode:SC_EOL_CR];
    ...
    // LF
    else if (lfOccurrences > crlfOccurrences && lfOccurrences > crOccurrences) {
        // Set EOL mode to LF
        [textEditor setEolMode:SC_EOL_LF];
...
- (wxNode *)retrieveNodeByName:(NSString *)nodeName {
    ...
    @try {
        for (NSInteger index = 0; index < [treeViewStructure numberOfRows];
             index++) {
        ...
        NSLog(@"wxDocument -> retrieveNodeByName Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (void)saveBookmarksPosition {
    @try {
        if ([[wxDefaults valueForKey:@"SaveBookmarks"] boolValue] &&
            ...
            NSInteger ret = [textEditor
                      FindText:[NSString
                                   stringWithFormat:@"%@[winxound_bookmarks",
                                                    languagecomment]
                MatchWholeWord:true
                     MatchCase:true
                    IsBackward:false
            ...
            if ([[textEditor getText] hasSuffix:@"\n"] ||
                ...
                // editor.textEditor.AppendText(editor.textEditor.NewLine +
            ...
            do {
                ...
                [textEditor
                    AppendText:[NSString stringWithFormat:@",%d", mBookLine]];
        ...
        [wxMAIN ShowMessageError:@"wxDocument -> saveBookmarksPosition:"
                           error:[NSString stringWithFormat:@"%@\n%@", [e name],
                                                            [e reason]]];
        // NSLog(@"wxDocument -> saveBookmarksPosition Error: %@ - %@", [e
        // name], [e reason]);
    }
}
...
- (void)showOnlineHelp:(BOOL)value {
    ...
    if(value == true)
    {
        ...
        [textEditorHost setFrame:NSMakeRect([textEditorHost frame].origin.x,
                                            [intelliTipHost bounds].size.height
    + 6, //3 [textEditorHost frame].size.width, [codeView frame].size.height -
    [intelliTipHost bounds].size.height - 10)];  //4
    }
    else
    {
        //HIDE INTELLITIP
    ...
    */
    ...
    if (value == true) {
        ...
        [intelliTipHost setHidden:false];
        [textEditorHost
            setFrame:NSMakeRect([textEditorHost frame].origin.x,
                                [intelliTipHost bounds].size.height + 6, // 3
                                [textEditorHost frame].size.width,
                                [codeView frame].size.height -
                                    [intelliTipHost bounds].size.height - 10 -
                                    offset)]; // 4
    } else {
        // HIDE INTELLITIP
        [intelliTipHost setHidden:true];
        [textEditorHost
            setFrame:NSMakeRect([textEditorHost frame].origin.x, 4,
                                [textEditorHost frame].size.width,
...
// Retrieve CsInstruments and CsScore positions (for Splitting view and Show Orc
// and Sco positions)
- (void)showOrcSco {
    if (![textEditor getIsSplitted])
        return;
    ...
    if (mFindPos > -1) {
        ...
        // ActiveEditor.textEditor.SetFirstVisibleLine(ActiveEditor.textEditor.GetLineNumberFromPosition(mFindPos),
        // 1);
        [textEditor
            setFirstVisibleLineAtView:[textEditor
                                          getLineNumberFromPosition:mFindPos]
                                 view:1];
    }
...
- (void)outlineView:(NSOutlineView *)outlineView
    ...
    if ([cell isKindOfClass:[ImageAndTextCell class]]) {
        ...
        if ([node.name isEqualToString:@"Bookmarks"])
            color = [NSColor colorWithCalibratedRed:.14
                                              green:.14
                                               blue:.14
                                              alpha:1];
        // image = exGrey;

        else if ([[[outlineView parentForItem:node] name]
                     isEqualToString:@"<CsOptions>"])
        ...
        // image = exGreen;
                     ...
                     isEqualToString:@"Bookmarks"])
            color = [NSColor colorWithCalibratedRed:.7
                                              green:.7
                                               blue:.7
                                              alpha:1];
        // image = exWhite;
            ...
            if ([node.name hasPrefix:@"f"]) {
                color = [NSColor colorWithCalibratedRed:.84
                                                  green:.63
                                                   blue:.39
                                                  alpha:1];
            } else if ([node.name hasPrefix:@"#def"]) {
                color = [NSColor colorWithCalibratedRed:.74
                                                  green:.25
                                                   blue:.73
                                                  alpha:1];
        ...
        // image = exRed;
            ...
            color = [NSColor colorWithCalibratedRed:.047
                                              green:.36
                                               blue:.53
                                              alpha:1];
        // image = exBlue;
        ...
        // image = exOrange;
            ...
            color = [NSColor colorWithCalibratedRed:.89
                                              green:.89
                                               blue:.89
                                              alpha:1];
        // image = exWhite;
        ...
        // autorelease]; NSImage* iconImage = //[[NSWorkspace sharedWorkspace]
        // iconForFile:urlStr];
...
- (void)findStructure:(NSString *)_text {
    ...
    @try {
        if ([[wxDefaults valueForKey:@"ExplorerShowOptions"] boolValue] &&
            ...
            if ([[NSThread currentThread] isCancelled])
                return;
            // NSLog(@"thread inside findStructure: %@", [NSThread
            // currentThread]);

            // OPTIONS
            NSInteger mStartCsOptions =
                [_text rangeOfString:@"<CsOptions>"
                             options:NSLiteralSearch
                               range:NSMakeRange(0, mFsEnd)]
                    .location;

            if (mStartCsOptions != NSNotFound) {
                mStartCsOptions += 11;
                NSInteger mEndCsOptions =
                    [_text rangeOfString:@"</CsOptions>"
                                 options:NSLiteralSearch
                                   range:NSMakeRange(mStartCsOptions,
                                                     mFsEnd - mStartCsOptions)]
                        .location;

                if (mEndCsOptions != NSNotFound) {
                    NSString *inText = [_text
                        substringWithRange:NSMakeRange(mStartCsOptions,
                                                       mEndCsOptions -
                                                           mStartCsOptions)];
                    NSArray *lines =
                        [inText componentsSeparatedByCharactersInSet:
                    ...
                    for (NSString *s in lines) {
                        ...
                        if (mFindRem == 0) {
                            ...
                            // mFsValue = [_text
                            // substringWithRange:NSMakeRange(mFsFindPos.location,
                            // mFsCurLineEnd - mFsFindPos.location)];
                            mFsValue = s;
                            mFsValue = [mFsValue
                                stringByTrimmingCharactersInSet:
                                    [NSCharacterSet
        ...
        if ([[wxDefaults valueForKey:@"ExplorerShowInstrMacros"] boolValue])
            [self findString:@"#define"
                      inText:_text
                    withNode:CsInstruments
                     isScore:false];

        // CSINTRUMENTS: OPCODE
        if ([[wxDefaults valueForKey:@"ExplorerShowInstrOpcodes"] boolValue])
            [self findString:@"opcode"
                      inText:_text
                    withNode:CsInstruments
                     isScore:false];

        // CSINTRUMENTS: INSTR
        if ([[wxDefaults valueForKey:@"ExplorerShowInstrInstruments"]
                boolValue])
            [self findString:@"instr"
                      inText:_text
                    withNode:CsInstruments
                     isScore:false];
    ...
    @catch (NSException *e) {
        NSLog(@"wxDocument -----------> findStructure Error: %@ - %@", [e name],
              [e reason]);
    }
    ...
    @finally {
        ...
        // Call bgThreadIsDone on the main thread (JOB is finished: notify
        // primary thread - refresh TreeViewStructure)
        [self performSelectorOnMainThread:@selector(bgThreadIsDone:)
                               withObject:Root
                            waitUntilDone:NO];
...
- (BOOL)findStringInScore:(NSString *)stringToFind
                   inText:(NSString *)_text
                 withNode:(wxNode *)passedNode {
    // CSSCORE
    ...
    if (mStartCsScore == NSNotFound) {
        ...
        if (![[self fileType] isEqualToString:@"CSound Sco Files"])
            return false;
        mStartCsScore = 0;
    }
    ...
    if (mStartCsScore != NSNotFound) {
        mStartCsScore += 8;
        NSInteger mEndCsScore =
            [_text rangeOfString:@"</CsScore>"
                         options:NSLiteralSearch
                           range:NSMakeRange(mStartCsScore,
                                             mFsEnd - mStartCsScore)]
                .location;
        ...
        if (mEndCsScore != NSNotFound) {
            NSString *inText = [_text
                substringWithRange:NSMakeRange(mStartCsScore,
                                               mEndCsScore - mStartCsScore)];
            NSArray *lines = [inText
            ...
            for (NSString *s in lines) {
                // SKIP INVALID LINES (REM)
                mFindRem = [[s stringByTrimmingCharactersInSet:
                                    [NSCharacterSet whitespaceCharacterSet]]
                               rangeOfString:@";"
                                     options:NSLiteralSearch]
                               .location;
                ...
                if (mFindRem == 0) {
                    ...
                    // substringWithRange:NSMakeRange(mFsFindPos.location,
                    // mFsCurLineEnd - mFsFindPos.location)];
                    f = [[s stringByTrimmingCharactersInSet:
                                 [NSCharacterSet whitespaceCharacterSet]]
                            rangeOfString:stringToFind
                                  options:NSLiteralSearch]
                            .location;
                    if (f == 0) {
                        mFsValue = s;
                        ...
                        if ([stringToFind isEqualToString:@"s"]) {
                            mFsKey = [NSString
                                stringWithFormat:@"%@ %d", mFsKey, index];
                            index++;
                        }
...
- (BOOL)findString:(NSString *)stringToFind
    ...
    @try {
        ...
        do {
            ...
            mFsFindPos =
                [_text rangeOfString:stringToFind
                             options:NSLiteralSearch
                               range:NSMakeRange(mFsStart, mFsEnd - mFsStart)];

            // Find only whole words !!! (Aggiungere eventualmente il rem ; come
            // possibile carattere iniziale
            if (mFsFindPos.location != NSNotFound &&
                [[[NSCharacterSet whitespaceAndNewlineCharacterSet] invertedSet]
                    characterIsMember:[_text
                                          characterAtIndex:mFsFindPos.location -
                                                           1]]) {
            ...
            if (mFsFindPos.location != NSNotFound) {
                ...
                // SKIP INVALID LINES (REM)
                startOfLine =
                    [_text rangeOfCharacterFromSet:[NSCharacterSet
                                                       newlineCharacterSet]
                                           options:NSBackwardsSearch
                                             range:NSMakeRange(
                                                       0, mFsFindPos.location)]
                        .location +
                    1;
                ...
                if (startOfLine != NSNotFound) {
                    ...
                    // NSString* temp = [_text
                    // substringWithRange:NSMakeRange(startOfLine,
                    // mFsFindPos.location - startOfLine)]; NSLog(@"temp: %@",
                    // temp); if([[temp
                ...
                // mFsCurLineEnd - mFsFindPos.location)];
                mFsValue =
                    [_text substringWithRange:NSMakeRange(startOfLine,
                                                          mFsCurLineEnd -
                                                              startOfLine)];
                mFsValue = [mFsValue
                    stringByTrimmingCharactersInSet:
        ...
        NSLog(@"wxDocument -----------> findString Error: %@ - %@", [e name],
              [e reason]);
        return false;
    }
...
- (NSString *)parseString:(NSString *)stringWithSpaces {
    NSArray *comps = [stringWithSpaces
        componentsSeparatedByCharactersInSet:[NSCharacterSet
...
- (void)refreshExplorer {
    @try {
        ...
        NSLog(@"wxDocument -> refreshExplorer Error: %@ - %@", [e name],
              [e reason]);
    }
}
...
- (void)bgThreadIsDone:(wxNode *)node {
    ...
    if (Options != nil) {
        if ([[[node.children objectAtIndex:0] children] count] > 0) {
            Options.children = [NSMutableArray
                arrayWithArray:[[node.children objectAtIndex:0] children]];
        } else {
    ...
    if (Instruments != nil) {
        if ([[[node.children objectAtIndex:1] children] count] > 0) {
            Instruments.children = [NSMutableArray
                arrayWithArray:[[node.children objectAtIndex:1] children]];
        } else {
    ...
    if (Score != nil) {
        if ([[[node.children objectAtIndex:2] children] count] > 0) {
            Score.children = [NSMutableArray
                arrayWithArray:[[node.children objectAtIndex:2] children]];
        } else {
            Score.children = nil;
        }
    }
    ...
    // Node was allocated by Self.FindStructure method so we must release it!!!
...
- (void)UpdateCurrentFileForCabbage {
    ...
    if ([[wxDefaults valueForKey:@"BringWinXoundToFrontForCabbage"]
            boolValue] == true) {
