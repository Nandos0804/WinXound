...
- (void)initializeCodeEditor {
    ...
    if (codeEditor == nil) {
        ...
        if (codeEditor == nil) {
            [wxMAIN
                ShowMessageError:@"wxCodeRepository -> displayCodeRepositoryAt:"
                           error:[NSString
                                     stringWithFormat:
                                         @"%@\n%@", @"Critical error!",
                                         @"codeEditor Initialization Failed!"]];
            return;
        }
...
- (void)initializeRepository {
    ...
    if (paths != nil) {
        if ([paths count] > 0) {
            ...
            winxoundPath =
                [[NSString stringWithFormat:@"%@/WinXound/CodeRepository",
                                            [paths objectAtIndex:0]] retain];

            // If the directory does not exist we create it and copy some
            // examples
            ...
            if ([fileManager fileExistsAtPath:winxoundPath] == false) {
                ...
                NSString *output = nil;
                NSArray *paths =
                    [[NSBundle mainBundle] pathsForResourcesOfType:@"udc"
                                                       inDirectory:nil];
                for (NSString *s in paths) {
                    output = [NSString
                        stringWithFormat:@"%@/%@.txt", winxoundPath,
                                         [[s lastPathComponent]
                ...
                // Copy udo files
                paths = [[NSBundle mainBundle] pathsForResourcesOfType:@"udo"
                                                           inDirectory:nil];
                for (NSString *s in paths) {
                    output = [NSString stringWithFormat:@"%@/%@", winxoundPath,
                                                        [s lastPathComponent]];
                    // NSLog(@"file: %@ to: %@", s, output);
                    [fileManager copyItemAtPath:s toPath:output error:nil];
                }
...
- (void)displayCodeRepositoryAt:(NSPoint)point {
    if (codeWindow == nil) {
        ...
        if(codeEditor == nil)
        {
            [wxMAIN ShowMessageError:@"wxCodeRepository ->
        displayCodeRepositoryAt:" error:[NSString stringWithFormat:@"%@\n%@",
                                      @"Critical error!", @"codeEditor
    ...
    if (codeWindow != nil) {
        ...
        if ([codeList selectedRow] == -1) {
            [codeList selectRowIndexes:[NSIndexSet indexSetWithIndex:0]
        ...
        [labelPath
            setStringValue:[NSString stringWithFormat:@"Repository path: %@",
                                                      winxoundPath]];
...
- (void)configureCodeEditor {
    ...
    [codeEditor setTabIndent:[[wxDefaults valueForKey:@"EditorTabIndent"]
                                 integerValue]];
    [codeEditor setZoom:0];
    [codeEditor setEolMode:SC_EOL_LF];

    //[codeEditor setScrollWidth: [[codeEditor getPrimaryView]
    ...
    [codeEditor setWordChars:@"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUV"
                             @"WXYZ0123456789.$_"];
    ...
    for (NSInteger mIndex = 0; mIndex < 34; mIndex++) {
        [codeEditor
            StyleSetFore:mIndex
               htmlcolor:[[wxMAIN getSettings] StyleGetForeColor:@"csound"
                                                     stylenumber:mIndex]];
        [codeEditor
            StyleSetBack:mIndex
               htmlcolor:[[wxMAIN getSettings] StyleGetBackColor:@"csound"
                                                     stylenumber:mIndex]];
        [codeEditor StyleSetBold:mIndex
                            bold:[[wxMAIN getSettings] StyleGetBold:@"csound"
                                                        stylenumber:mIndex]];
        [codeEditor
            StyleSetItalic:mIndex
                    italic:[[wxMAIN getSettings] StyleGetItalic:@"csound"
                                                    stylenumber:mIndex]];
    }
    ...
    [codeEditor setCaretFore:[[wxMAIN getSettings] StyleGetForeColor:@"csound"
                                                         stylenumber:32]];
}
...
- (IBAction)wxInputBoxPanelOK:(id)sender {
    ...
    if ([[textEditor getSelectedText] length] > 0) {
        ...
        NSString *path =
            [NSString stringWithFormat:@"%@/%@.txt", winxoundPath, filename];
        ...
        for (NSInteger i = 0; i < [codeList numberOfRows]; i++) {
            ...
            // NSLog([cell title]);
            if ([[cell title]
                    isEqualToString:[NSString stringWithFormat:@"%@.txt",
                                                               filename]]) {
                index = i;
                break;
            }
        ...
        //[nset release];
        [codeList selectRowIndexes:[NSIndexSet indexSetWithIndex:index]
              byExtendingSelection:NO];
...
- (IBAction)codeInsertText:(id)sender {
    ...
    if ([[textEditor getSelectedText] length] > 0) {
        ...
        //[textEditor setCaretPosition:([textEditor getCaretPosition] + [text
        //length])];
...
- (IBAction)codeRemoveFile:(id)sender {
    ...
    NSString *path =
        [NSString stringWithFormat:@"%@/%@", winxoundPath, filename];
    ...
    if (selRow < [files count])
        [codeList selectRowIndexes:[NSIndexSet indexSetWithIndex:selRow]
              byExtendingSelection:NO];
    // else
    //	[codeList selectRowIndexes:[NSIndexSet indexSetWithIndex:selRow-1]
    //byExtendingSelection:NO];
...
- (void)refreshList {
    if (codeWindow != nil) {
        ...
        NSArray *temp = [[NSFileManager defaultManager]
            contentsOfDirectoryAtPath:winxoundPath
                                error:nil];

        for (NSString *filename in temp) {
            // Skip hidden files and files does not terminate with txt or udo
            // extension
...
- (IBAction)codeRestoreDefaultFiles:(id)sender {
    ...
    NSInteger ret =
        [wxMAIN ShowMessage:@"WinXound Code Repository alert!"
            informativeText:
                @"This operation will restore the default WinXound Repository "
                @"files. "
                 "If you have modified some original files, all changes will "
                 "be lost. \n"
    ...
    if (paths != nil) {
        if ([paths count] > 0) {
            ...
            if ([fileManager fileExistsAtPath:winxoundPath] == false) {
                ...
                winxoundPath = [[NSString
                    stringWithFormat:@"%@/WinXound/CodeRepository",
                                     [paths objectAtIndex:0]] retain];
...
- (id)tableView:(NSTableView *)tableView
    ...
    @catch (NSException *e) {
        NSLog(@"wxCodeRepository -> objectValueForTableColumn Error: %@ - %@",
              [e name], [e reason]);
    }
