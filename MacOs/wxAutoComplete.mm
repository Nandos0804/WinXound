...
- (void)FillTableWithOpcodes {
    ...
    for (NSString *mString in lines) {
        NSString *temp = [mString stringByReplacingOccurrencesOfString:@"\""
                                                            withString:@""];
        NSArray *split = [temp componentsSeparatedByString:@";"];
        NSString *text = [split objectAtIndex:0];

        if ([opcodes containsObject:text] == false && [text length] > 0) {
...
- (void)displayAutocompleteAt:(NSPoint)point {
    ...
    if (CSoundAutocompleteWindow != nil) {
        ...
        [TableViewOpcodes selectRowIndexes:[NSIndexSet indexSetWithIndex:0]
                      byExtendingSelection:NO];
        if ([[textEditor getCurrentWord] length] > 0) {
            // NSInteger index = [opcodes indexOfObject:[textEditor
            // getCurrentWord]];
            for (NSInteger index = 0; index < [opcodes count]; index++) {
                if ([[opcodes objectAtIndex:index]
                        hasPrefix:[textEditor getCurrentWord]]) {
                    //[TableViewOpcodes selectRow:index
...
- (IBAction)wxDoubleClickInsertText:(id)sender {
    ...
    if ([TableViewOpcodes selectedRow] < 0)
        return;
...
- (IBAction)wxAutocompleteInsertText:(id)sender {
    ...
    if ([[temp title] isEqualToString:@"enter"]) {
        ...
        //[textEditor AddText:[opcodes objectAtIndex:[TableViewOpcodes
        // selectedRow]]];
        [self insertAutocompleteString:text space:@"" opcode:text];
    }
        ...
        NSString *opcode =
            [opcodes objectAtIndex:[TableViewOpcodes selectedRow]];
        ...
        if ([description length] > 0) {
            ...
            if ([[synopsis lowercaseString] rangeOfString:@"not available"]
                    .location != NSNotFound)
                return;
...
- (void)insertAutocompleteString:(NSString *)text
    ...
    [[textEditor getFocusedEditor] setGeneralProperty:SCI_BEGINUNDOACTION
                                            parameter:0
                                                value:0];
    {
        if (start != pos) {
    ...
    [[textEditor getFocusedEditor] setGeneralProperty:SCI_ENDUNDOACTION
                                            parameter:0
                                                value:0];
...
- (id)tableView:(NSTableView *)tableView
    ...
    @catch (NSException *e) {
        NSLog(@"wxAutoComplete -> objectValueForTableColumn Error: %@ - %@",
              [e name], [e reason]);
    }
