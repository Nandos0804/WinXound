...
- (NSString *)ImportORCSCO:(NSString *)filename //(ref string SciEditFileName)
{
    ...
    // string OrcFileName = SciEditFileName.Remove(SciEditFileName.Length - 4,
    // 4) + ".orc"; string ScoFileName =
    // SciEditFileName.Remove(SciEditFileName.Length - 4, 4) + ".sco";
    NSString *OrcFileName = [NSString
        stringWithFormat:@"%@.orc", [filename stringByDeletingPathExtension]];
    NSString *ScoFileName = [NSString
        stringWithFormat:@"%@.sco", [filename stringByDeletingPathExtension]];
    ...
    if ([fileManager fileExistsAtPath:OrcFileName]) {
        ...
        // tempOrc = [NSString stringWithContentsOfFile:OrcFileName
    ...
    if ([fileManager fileExistsAtPath:ScoFileName]) {
        ...
        // tempSco = [NSString stringWithContentsOfFile:ScoFileName
        // encoding:NSUTF8StringEncoding error:nil];
        tempSco = [wxMAIN getStringFromFilename:ScoFileName];
    }

    NSString *tempString = [NSString stringWithFormat:@"<CsoundSynthesizer>\n\n"
                                                       "<CsOptions>\n"
                                                       "-W -odac\n"
                                                       "</CsOptions>\n\n"
                                                       "<CsInstruments>\n"
...
- (void)internalImportORC:(TextEditorView *)textEditor
    ...
    [textEditor setSelection:startInstr + 15 end:endInstr];
...
- (void)internalImportSCO:(TextEditorView *)textEditor
             fromFilename:(NSString *)filename {
    ...
    [textEditor setSelection:startScore + 10 end:endScore];
...
- (void)internalExportOrcSco:(TextEditorView *)textEditor
    ...
    @try {
        ...
        // Search for <CsInstruments> section: start and end
        // Find <CsInstruments>
        startSection = [self findTextPositionWithNSString:text
        ...
        // Find </CsInstruments>
        endSection = [self findTextPositionWithNSString:text
                                           stringToFind:@"</CsInstruments>"];
        ...
        NSRange sectionRange =
            NSMakeRange(startSection, endSection - startSection);
        ...
        NSString *outputFilename =
            [NSString stringWithFormat:@"%@.orc", filename];
        [tempOrc writeToFile:outputFilename
                  atomically:YES
                    encoding:NSUTF8StringEncoding
                       error:nil];

        outputFilename = [NSString stringWithFormat:@"%@.sco", filename];
        [tempSco writeToFile:outputFilename
                  atomically:YES
                    encoding:NSUTF8StringEncoding
                       error:nil];
...
- (void)internalExportOrc:(TextEditorView *)textEditor
               toFilename:(NSString *)filename {
...
- (void)internalExportSco:(TextEditorView *)textEditor
               toFilename:(NSString *)filename {
    ...
    @try {
        ...
        // Write Sco to disc
        NSString *outputFilename =
            [NSString stringWithFormat:@"%@.sco", filename];
        [tempSco writeToFile:outputFilename
                  atomically:YES
                    encoding:NSUTF8StringEncoding
                       error:nil];
...
- (NSInteger)findTextPositionWithNSString:(NSString *)_text
    ...
    @try {
        ...
        do {
            ...
            ////Find only whole words !!! (Aggiungere eventualmente il rem ;
            ///come possibile carattere iniziale
            ...
            //	[[[NSCharacterSet whitespaceAndNewlineCharacterSet] invertedSet]
            //characterIsMember:
            //	 [_text characterAtIndex:mFsFindPos.location +
            //mFsFindPos.length]])
            //{
        ...
        NSLog(@"wxImportExport -> findTextPositionWithNSString Error: %@ - %@",
              [e name], [e reason]);
    }
...
//--------------------------------------------------------------------------------------------------
- (void)BrowseFile:(NSString *)extension
         forWindow:(NSWindow *)sheetWindow
            editor:(TextEditorView *)textEditor {
    ...
    // MODAL WINDOW
    // NSInteger result = [openPanel runModalForDirectory:nil file:nil
    // types:nil];
...
- (void)SaveFile:(NSString *)filename
    ...
    [spanel beginSheetForDirectory:nil // NSHomeDirectory()
                    ...
                    modalForWindow:sheetWindow
                     modalDelegate:self
                    didEndSelector:@selector(didEndSaveSheet:
                                                  returnCode:contextInfo:)
                       contextInfo:textEditor];
}
