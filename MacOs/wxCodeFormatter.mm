...
- (void)formatCode:(TextEditorView *)textEditor
    ...
    @try {
        ...
        for (NSInteger i = mStart; i < mEnd; i++) {
            ...
            lineLength = [textEditor GetLineEndPosition:i] -
                         [textEditor getPositionFromLineNumber:i];
            ...
            // Retrieve text of the line (trim whitespaces and newline chars)
            // //mEditor.GetTextLine(i).TrimStart();
            textline =
                [line stringByTrimmingCharactersInSet:
                          [NSCharacterSet whitespaceAndNewlineCharacterSet]];
            ////textline = [line stringByTrimmingCharactersInSet:[NSCharacterSet
            ...
            NSArray *words =
                [textline componentsSeparatedByCharactersInSet:separator];
            ...
            for (NSString *word in words) {
                ...
                if ([word rangeOfString:@";"].location != NSNotFound &&
                    ...
                    remIndex = [word rangeOfString:@";"]
                                   .location; // textline.IndexOf(";");
                    break;
                }
                ...
                //	&& isMultilineRem == false)
                //	isScore = true;
                if ([word isEqualToString:@"<CsScore>"] &&
                    isSinglelineRem == false && isMultilineRem == false)
                    isScore = true;
                ...
                if ([word isEqualToString:@"instr"] ||
                    ...
                    // if (word == "instr" && isSinglelineRem == false &&
                    // isMultilineRem == false) 	isInstrument = true;
                    if ([word isEqualToString:@"instr"] &&
                        isSinglelineRem == false && isMultilineRem == false)
                        isInstrument = true;
                    // if (word == "endin" && isSinglelineRem == false &&
                    // isMultilineRem == false) 	isInstrument = false;
                    if ([word isEqualToString:@"endin"] &&
                        isSinglelineRem == false && isMultilineRem == false)
                        isInstrument = false;

                    // if (wxGlobal.Settings.CodeFormat.FormatInstruments)
                    if ([[wxDefaults valueForKey:@"CodeFormatInstruments"]
                            boolValue]) {
                        textline = [self parseString:textline];
                        // Style 1: Add tab to start
                        // if (wxGlobal.Settings.CodeFormat.InstrumentsType ==
                        // 0)
                        if ([[wxDefaults
                                valueForKey:@"CodeFormatInstrumentsType"]
                                integerValue] == 0) {
                            // textline = "\t" + textline;
                            textline =
                                [NSString stringWithFormat:@"\t%@", textline];
                            // mEditor.ReplaceTarget(startpos,
                            // mEditor.GetLineLength(i), textline); [finalText
                            //appendString:[NSString stringWithFormat:@"%@\n",
                            //textline]];
                            [textEditor
                                ReplaceTarget:[textEditor
                                                  getPositionFromLineNumber:i]
                                       length:lineLength //[textEditor
                                                         //getLineLength:i]
                ...
                //		 [word rangeOfString:@"nchnls"].location != NSNotFound)
                    ...
                    if ([[wxDefaults valueForKey:@"CodeFormatHeader"]
                            boolValue]) {
                        // textline = Regex.Replace(textline, @"\s+", " ") +
                        // newline;
                        textline = [NSString
                            stringWithFormat:@"%@",
                                             [self parseString:textline]];

                        // tempindex = textline.IndexOf("=");
                        tempindex = [textline rangeOfString:@"="].location;
                        if (tempindex > -1)
                            // textline = "\t" + textline.Substring(0,
                            // tempindex).Trim() +
                            //"\t" + textline.Substring(tempindex).Trim() +
                            //newline;
                            textline = [NSString
                                stringWithFormat:
                                    @"\t%@\t%@",
                ...
                //		 word == "endif")
                    ...
                    // mEditor.ReplaceTarget(startpos, mEditor.GetLineLength(i),
                    // textline); line = textline; [finalText
                    //appendString:[NSString stringWithFormat:@"%@\n",
                    //textline]];
                    [textEditor
                        ReplaceTarget:[textEditor getPositionFromLineNumber:i]
                ...
                // else if (KeyWords.Contains(word) || word == "=")
                    ...
                    if ([[wxDefaults valueForKey:@"CodeFormatInstruments"]
                        ...
                        NSArray *split = [textline
                            componentsSeparatedByCharactersInSet:separator];
                        // string mCompose = "";
                        NSMutableString *mCompose =
                        ...
                        //[finalText appendString:[NSString
                        //stringWithFormat:@"%@\n", mCompose]];
                        [textEditor
                            ReplaceTarget:[textEditor
                                              getPositionFromLineNumber:i]
            ...
            if (isSinglelineRem == true && isMultilineRem == false &&
                isScore == true) {

                // if (CheckScoreForGoodLine(textline) == false) continue;
                if ([self CheckScoreForGoodLine:textline] == false)
                    continue;
                ...
                tempString =
                    [tempString stringByReplacingOccurrencesOfString:@" "
                                                          withString:@"\t"];
                // textline = Regex.Replace(tempString, @"\s+", " ") +
                // textline.Substring(remIndex);
                textline = [NSString
                    stringWithFormat:@"%@%@", tempString,
                                     [textline substringFromIndex:remIndex]];

            } else if (isSinglelineRem == false && isMultilineRem == false &&
                       isScore == true) {
                // if (CheckScoreForGoodLine(textline) == false) continue;
                if ([self CheckScoreForGoodLine:textline] == false)
                    continue;
            ...
            if (isReplaced == false) {
                // mEditor.ReplaceTarget(startpos, mEditor.GetLineLength(i),
                // textline); [finalText appendString:[NSString
                //stringWithFormat:@"%@\n", textline]];
                [textEditor
                    ReplaceTarget:[textEditor getPositionFromLineNumber:i]
                           length:lineLength //[textEditor getLineLength:i]
        ...
        [textEditor
            setCaretPosition:[textEditor getPositionFromLineNumber:mEnd]];
...
- (BOOL)CheckScoreForGoodLine:(NSString *)textline {
    ...
    if ([textline hasPrefix:@"i"] &&
        [[wxDefaults valueForKey:@"CodeFormatScoreInstruments"] boolValue] ==
            false)
        return false;
...
- (void)showOptionsWindow {
    ...
    //		modalDelegate:self
    //	   didEndSelector:nil
    ////@selector(sheetDidFinish:returnCode:contextInfo:) 		  contextInfo:nil];
}
...
- (IBAction)wxFormatterRESET:(id)sender {
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"CodeFormatHeader"];
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"CodeFormatInstruments"];
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"CodeFormatFunctions"];
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"CodeFormatScoreInstruments"];
    [wxDefaults setValue:[NSNumber numberWithBool:true]
                  forKey:@"CodeFormatTempo"];
    [wxDefaults setValue:[NSNumber numberWithInteger:0]
                  forKey:@"CodeFormatInstrumentsType"];
...
- (void)FormatExample {
    ...
    // textEditorExample.TabIndent =
    // wxGlobal.Settings.EditorProperties.DefaultTabSize [textView
    //setFont:[NSFont fontWithName:@"Andale Mono" size:12]];

    //[[textView textStorage] beginEditing];

    [[[textView textStorage] mutableString]
        appendString:@"<CsoundSynthesizer>\n"
                      "<CsOptions>\n"
                      "-W -odac\n"
                      "</CsOptions>\n"
                      "<CsInstruments>\n"
    ...
    if ([[wxDefaults valueForKey:@"CodeFormatHeader"] boolValue]) {
        [[[textView textStorage] mutableString]
            appendString:@"\tsr     = 44100\n"
                          "\tkr     = 4410\n"
                          "\tksmps  = 10\n"
                          "\tnchnls = 1\n\n"];
    } else {
        [[[textView textStorage] mutableString] appendString:@"sr = 44100\n"
                                                              "kr = 4410\n"
                                                              "ksmps = 10\n"
                                                              "nchnls = 1\n\n"];
    }
    ...
    if ([[wxDefaults valueForKey:@"CodeFormatInstruments"] boolValue]) {
        ...
        if ([[wxDefaults valueForKey:@"CodeFormatInstrumentsType"]
                integerValue] == 0) {
            [[[textView textStorage] mutableString]
                appendString:@"\tinstr 1\n"
    ...
    if ([[wxDefaults valueForKey:@"CodeFormatScoreInstruments"] boolValue]) {
        [[[textView textStorage] mutableString] appendString:@"i1\t0\t3\n"];
    } else {
        [[[textView textStorage] mutableString] appendString:@"i1 0 3\n"];
    }
    ...
    if ([[wxDefaults valueForKey:@"CodeFormatTempo"] boolValue]) {
        [[[textView textStorage] mutableString]
            appendString:@"t\t0\t60\t40\t60\t45\t30\n"];
    } else {
        [[[textView textStorage] mutableString]
            appendString:@"t 0 60 40 60 45 30\n"];
    }
