...
- (void)compile:(NSString *)compilerName
        ...
        if (parSep != nil) {
            if ([parSep count] > 1) // Multiple flags
            {
                ...
                for (NSString *s in parSep) {
                    if ([s length] > 0) {
                        // if([_compilerName lowercaseString]
                        // rangeOfString:@"csound"].location == NSNotFound)
    ...
    // NSLog(@"Compiler arguments: %@ - Environment: %@", [task arguments],
    // [task environment]); DEBUG:
    ...
    // asynchronously, and notify us via the callback registered above when we
    // signed up as an observer.  The file handle will send a
    // NSFileHandleReadCompletionNotification when it has data that is
    // available.
    ...
    if (_output != nil) {
        ...
        if (showArguments && [arguments count] > 0) {
            ...
            if ([[_compilerName lowercaseString] rangeOfString:@"csound"]
                    .location != NSNotFound)
                startIndex = 1;

            [[[_output textStorage] mutableString]
                appendString:@"Compiler arguments (added by WinXound):\n"];
...
// This method is called asynchronously when data is available from the task's
// file handle. We just pass the data along to the controller as an NSString.
- (void)getData:(NSNotification *)aNotification {
    NSData *data = [[aNotification userInfo]
        objectForKey:NSFileHandleNotificationDataItem];
    // If the length of the data is zero, then the task is basically over -
    // there is nothing more to get from the handle so we may as well shut down.

    if (data == nil) // We're finished here
        [self TaskCompletedNotification:nil];

    if ([data length]) {
        // Send the data on to the controller; we can't just use
        // +stringWithUTF8String: here because -[data bytes] is not necessarily
        ...
        @try {
            if (_output != nil) {
                [_output setFont:[NSFont fontWithName:_fontName
                                                 size:_fontSize]];

                [[[_output textStorage] mutableString]
                    appendString:[[[NSString alloc]
                                     initWithData:data
    ...
    // we need to schedule the file handle go read more data in the background
    // again.
    //[[aNotification object] readInBackgroundAndNotify];
}
...
- (void)TaskCompleted:(BOOL)stoppedByUser {
    ...
    @try {
        ...
        if (task != nil) {
            ...
            if (_output != nil && task != nil) {
                while ((data = [[[task standardOutput] fileHandleForReading]
                            availableData]) &&
                       [data length]) {
    ...
    if (_output != nil) {
        ...
        if (stoppedByUser)
            [[[_output textStorage] mutableString]
                appendString:@"------- Terminated by user -------\n"];

        [[[_output textStorage] mutableString]
            appendString:@"------- Compiler Ended -------"];
    ...
    if (_output != nil && _owner != nil) {
        // wxDocument: if _owner is wxDocument and the user hasn't stopped the
        // compiler, we must CHECK FOR ERRORS or SOUNDFILES (call self
        // compilerCompleted method)
...
// If the task ends, there is no more data coming through the file handle even
// when the notification is sent, or the process object is released, then this
// method is called.
- (void)stopProcess {
    ...
    @try {
        if ([[_compilerName lowercaseString] rangeOfString:@"csound"]
            ...
            if ([errorLine length] > 0) {
                ...
                if ([[wxDefaults valueForKey:@"OpenSoundFileWith"]
                        integerValue] > 0) {
                    // 0 = nothing - 1 = Media Player - 2 = Wave Editor
                    if ([[NSFileManager defaultManager]
                            fileExistsAtPath:soundLine]) {
                        // MEDIA PLAYER = 1
                        if ([[wxDefaults valueForKey:@"OpenSoundFileWith"]
                                integerValue] == 1) {
                            if ([[wxDefaults valueForKey:@"DefaultWavePlayer"]
                                    integerValue] == 1) {
        ...
        NSLog(@"wxCompiler -> compilerCompleted Error: %@ - %@", [e name],
              [e reason]);
    }
}
...
- (NSString *)findError:(NSString *)text {
                ...
                if (mFsFindPos.location != NSNotFound) {
                    if (returnString == nil) {
                        returnString = [currentLine
                            substringFromIndex:mFsFindPos.location + 4];
    ...
    if (returnString != nil) {
        if (_output != nil && mStart > 0) {
            NSRange range =
                NSMakeRange(mStart, [[_output string] length] - mStart);
            // NSRange range = NSMakeRange (mStart, 16);
            [_output setTextColor:[NSColor colorWithCalibratedRed:0.7
                                                            green:0
                                                             blue:0
                                                            alpha:1]
                            range:range];
...
- (NSString *)findSounds:(NSString *)text {
    ...
    @try {
        ...
        // writing 2048-byte blks of shorts to /Users/teto/Desktop/fm_01.wav
        // (WAV) 1034 2048-byte soundblks of shorts written to
        // /Users/teto/Desktop/fm_01.wav (WAV)
        ...
        for (NSInteger index = 0; index < [lines count]; index++) {
            ...
            if ([currentLine rangeOfString:@"written"].location == NSNotFound) {
                if ([currentLine rangeOfString:@"writing"].location ==
                    NSNotFound)
                    continue;
            }
            ...
            if (mFindPos != NSNotFound) {
                ...
                if (mFindStart != NSNotFound) {
                    ...
                    NSString *mLine = [currentLine
                        substringToIndex:mFindPos + extensionLength];
                    ...
                    mFindStart =
                        [currentLine rangeOfString:@"writing"].location;
                    if (mFindStart != NSNotFound) {
                        ...
                        if (mFindStart != NSNotFound) {
                            NSString *mLine = [currentLine
                                substringToIndex:mFindPos + extensionLength];
                            tempString =
                                [mLine substringFromIndex:mFindStart + 3];
                            return tempString;
                        }
                    }
                }
