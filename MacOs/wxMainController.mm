...
static void feCallback(ConstFSEventStreamRef streamRef,
    ...
    for (i = 0; i < numEvents; i++) {
        ...
        if ([[[NSString stringWithUTF8String:paths[i]] lowercaseString]
                rangeOfString:@"trash"]
                .location != NSNotFound) {
    ...
    @try {
        while (obj = [enumerator nextObject]) {
            ...
            if ([items count] > 2) {
                ...
                NSString *key = [k stringByReplacingOccurrencesOfString:@"\""
                                                             withString:@""];
                NSString *value = [NSString
                    stringWithFormat:@"%@;%@;%@", [items objectAtIndex:1],
...
- (BOOL)applicationShouldHandleReopen:(NSApplication *)sender
    ...
    if ([[[NSDocumentController sharedDocumentController] documents] count] ==
            0 &&
        [HelpWindow isVisible] == false && [wxPLAYER IsWindowVisible] == false)
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    NSString *versionStr =
        [self GetInfoValueForKey:@"CFBundleShortVersionString"];
    [version
        setStringValue:[NSString stringWithFormat:@"version %@", versionStr]];
    ...
    CFArrayRef _pathsToWatch =
        CFArrayCreate(NULL, (const void **)&_path, 1, NULL);
    ...
    _stream = FSEventStreamCreate(
        NULL, &feCallback, _context, _pathsToWatch,
...
- (void)UpdateAdditionalFlagsMenu {
    ...
    for (NSMenuItem *mi in
         [menuCompileExternalWithAdditionalOptions itemArray]) {
    ...
    for (NSString *line in lines) {
        ...
        if ([line length] > 0) {
            [menuCompileWithAdditionalOptions
                addItemWithTitle:line
                          action:@selector(CompileWithAdditionalFlagsClicked:)
                   keyEquivalent:@""];

            [menuCompileExternalWithAdditionalOptions
                addItemWithTitle:line
                          action:@selector
    ...
    if (paths != nil) {
        if ([paths count] > 0) {
            NSString *winxoundPath =
                [[NSString stringWithFormat:@"%@/WinXound/Temp",
            ...
            if ([[NSFileManager defaultManager]
                ...
                while (NSString *file = [en nextObject]) {
                    [[NSFileManager defaultManager]
                        removeItemAtPath:
                            [winxoundPath stringByAppendingPathComponent:file]
                                   error:nil];
                }
            }
        }
...
// OVERRIDE NSDOCUMENTCONTROLLER clearRecentDocuments (to check documents for
// RecentFiles tableView)
...
- (void)applySettings {
    ...
    // sharedUserDefaultsController]; [defaultsController save:self];
}
...
- (void)ShowMessageError:(NSString *)title error:(NSString *)error;
{
    ...
    NSInteger ret =
        [self ShowMessage:[NSString
                              stringWithFormat:@"WinXound Error:\n%@", title]
            informativeText:
                [NSString
                    stringWithFormat:
                        @"%@\n\n"
                         "If you would like to signal this error message to "
                         "the author "
                         "please click the 'OK' button (thanks for your help).",
                        error]
              defaultButton:@"OK"
    ...
    if (ret == NSAlertFirstButtonReturn) // OK BUTTON PRESSED - Send an email to
                                         // authors
    {
        // NSLog(@"OK BUTTON PRESSED");
        //%0a=linefeed &0d=carriage return

        NSString *EmailLink = [NSString
            stringWithFormat:@"mailto:stefano_bonetti@tin.it?"
                              "subject=WinXound Error Notification [OSX]"
                              "&body=Title: %@\nError: %@",
                             title, error];

        NSString *finalLink = [EmailLink
...
+ (void)setupDefaults {
    ...
    // CSOUND
    NSString *userCSoundStylesPath =
        [[NSBundle mainBundle] pathForResource:@"CSoundSyntax" ofType:@"plist"];
    NSDictionary *userCSoundStylesDict =
        [NSDictionary dictionaryWithContentsOfFile:userCSoundStylesPath];
    [userDefaultsValuesDict setValue:userCSoundStylesDict
                              forKey:@"CSoundStyles"];

    // PYTHON
    NSString *userPythonStylesPath =
        [[NSBundle mainBundle] pathForResource:@"PythonSyntax" ofType:@"plist"];
    NSDictionary *userPythonStylesDict =
        [NSDictionary dictionaryWithContentsOfFile:userPythonStylesPath];
    [userDefaultsValuesDict setValue:userPythonStylesDict
                              forKey:@"PythonStyles"];

    // LUA
    NSString *userLuaStylesPath =
        [[NSBundle mainBundle] pathForResource:@"LuaSyntax" ofType:@"plist"];
    NSDictionary *userLuaStylesDict =
        [NSDictionary dictionaryWithContentsOfFile:userLuaStylesPath];
    ...
    NSString *valuesPath = [[NSBundle mainBundle] pathForResource:@"analysis"
                                                           ofType:@"plist"];
    NSDictionary *analysisValuesDict =
        [NSDictionary dictionaryWithContentsOfFile:valuesPath];
...
- (void)createNewCsoundFileWithContentOfString:(NSString *)text {
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"CSound Files"
                             error:nil];
    if (doc != nil) {
...
- (BOOL)checkForOrcSco:(NSString *)filename {
    if ([[[filename lastPathComponent] lowercaseString] hasSuffix:@".orc"] ||
        ...
        if ([[wxDefaults valueForKey:@"OrcScoImport"] integerValue] ==
            0) // SHOW MESSAGE
        {
            ...
            [alert setInformativeText:
                       @"Would you like to convert your Orc/Sco file to a new "
                       @"Csd document?\n\n"
                        "To change the default Orc/Sco import action please "
                        "look at:\n"
        ...
        } else if ([[wxDefaults valueForKey:@"OrcScoImport"] integerValue] ==
                   1) // CONVERT TO CSD
        {
            NSString *temp = [wxIMPEXP ImportORCSCO:filename];
            [self createNewCsoundFileWithContentOfString:temp];
...
- (IBAction)wxNewOrcFile:(id)sender {
    ...
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"CSound Orc Files"
                             error:nil];
    if (doc != nil) {
        [[NSDocumentController sharedDocumentController] addDocument:doc];
        [doc makeWindowControllers];
...
- (IBAction)wxNewScoFile:(id)sender {
    ...
    // openUntitledDocumentAndDisplay:NO error:nil];
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"CSound Sco Files"
                             error:nil];
    if (doc != nil) {
        [[NSDocumentController sharedDocumentController] addDocument:doc];
        [doc makeWindowControllers];
...
- (IBAction)wxNewCabbageFile:(id)sender {
    ...
    // openUntitledDocumentAndDisplay:NO error:nil];
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"CSound Cabbage Files"
                             error:nil];
    if (doc != nil) {
        [[NSDocumentController sharedDocumentController] addDocument:doc];
        [doc makeWindowControllers];
...
- (IBAction)wxNewPythonFile:(id)sender {
    ...
    // openUntitledDocumentAndDisplay:NO error:nil];
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"Python Files"
                             error:nil];
    if (doc != nil) {
        [[NSDocumentController sharedDocumentController] addDocument:doc];
...
- (IBAction)wxNewLuaFile:(id)sender {
    ...
    // openUntitledDocumentAndDisplay:NO error:nil];
    NSDocument *doc = [[NSDocumentController sharedDocumentController]
        makeUntitledDocumentOfType:@"Lua Files"
                             error:nil];
    if (doc != nil) {
        [[NSDocumentController sharedDocumentController] addDocument:doc];
...
- (IBAction)wxOpenRecent:(id)sender {
    ...
    NSUInteger *buf = (NSUInteger *)calloc(
        bufSize,
        sizeof(NSUInteger)); // new unsigned int[bufSize]; //(NSUInteger
                             // *)calloc(bufSize, sizeof(NSUInteger));

    @try {
        NSRange range =
            NSMakeRange([indexSet firstIndex], [indexSet lastIndex] + 1);
        [indexSet getIndexes:buf maxCount:bufSize inIndexRange:&range];
        for (NSUInteger i = 0; i != bufSize; i++) {
            NSUInteger index = buf[i];
            if ([self checkForOrcSco:[[files objectAtIndex:index] path]] ==
                false)
                [[NSDocumentController sharedDocumentController]
                    openDocumentWithContentsOfURL:[files objectAtIndex:index]
                                          display:YES
                                            error:nil];
        }
...
- (IBAction)wxOpen:(id)sender {
    ...
    // NSLog(@"WXOPEN");
    NSArray *filenames = [[NSDocumentController sharedDocumentController]
...
- (IBAction)wxShowLineNumbers:(id)sender {
    // BOOL currentValue = [[wxDefaults valueForKey:@"EditorShowLineNumbers"]
    // boolValue];
    BOOL newValue =
...
- (IBAction)wxShowExplorer:(id)sender {
    // BOOL currentValue = [[wxDefaults valueForKey:@"EditorShowExplorer"]
    // boolValue];
...
- (IBAction)wxShowOnlineHelp:(id)sender {
    // BOOL currentValue = [[wxDefaults valueForKey:@"EditorShowIntellitip"]
    // boolValue];
    BOOL newValue =
...
- (IBAction)wxShowToolbar:(id)sender {
    // BOOL currentValue = [[wxDefaults valueForKey:@"EditorShowToolbar"]
    // boolValue];
...
- (IBAction)wxCallMediaPlayer:(id)sender {
    if ([[wxDefaults valueForKey:@"DefaultWavePlayer"] integerValue] == 1) {
        if ([[NSFileManager defaultManager]
                fileExistsAtPath:@"/Applications/QuickTime Player.app"]) {
            [[NSWorkspace sharedWorkspace]
                launchApplication:@"/Applications/QuickTime Player.app"];
            return;
        }
    }
...
- (IBAction)wxCallWaveEditor:(id)sender {
    if (![[NSFileManager defaultManager]
            fileExistsAtPath:[wxDefaults valueForKey:@"WaveEditorPath"]]) {
    ...
    [[NSWorkspace sharedWorkspace]
        launchApplication:[wxDefaults valueForKey:@"WaveEditorPath"]];
}

- (IBAction)wxCallCalculator:(id)sender {
    if ([[NSFileManager defaultManager]
            fileExistsAtPath:@"/Applications/Calculator.app"])
        [[NSWorkspace sharedWorkspace]
            launchApplication:@"/Applications/Calculator.app"];
}
...
- (IBAction)goHome:(id)sender {
    ...
    if (urlText != nil) {
        if ([[NSFileManager defaultManager]
                fileExistsAtPath:urlText]) // && [urlText length] > 0)
        {
            [[helpBrowser mainFrame]
...
- (IBAction)wxShowCSoundFlagsHelp:(id)sender {
    ...
    NSString *path = [[wxDefaults valueForKey:@"CSoundHelpHtmlPath"]
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:mFile]) {
        [[helpBrowser mainFrame]
            loadRequest:[NSURLRequest
                            requestWithURL:[NSURL URLWithString:mFile]]];
        [self wxShowHelp:self];
    }
}
...
- (IBAction)wxShowCSoundOpcodesHelp:(id)sender {
    ...
    NSString *path = [[wxDefaults valueForKey:@"CSoundHelpHtmlPath"]
        stringByDeletingLastPathComponent];
    NSString *mFile =
        [NSString stringWithFormat:@"%@/PartReference.html", path];
    if ([[NSFileManager defaultManager] fileExistsAtPath:mFile]) {
        [[helpBrowser mainFrame]
            loadRequest:[NSURLRequest
                            requestWithURL:[NSURL URLWithString:mFile]]];
...
- (void)showOpcodeHelp:(NSString *)opcode {
    ...
    if ([Opcodes valueForKey:opcode] != nil) {
        if (path != @"") {
            ...
            // PyCall family
            if ([opcode
                    hasPrefix:@"pycall"] || //(gCurWord.StartsWith("pycall") ||
                [opcode
                    hasPrefix:@"pylcall"]) // gCurWord.StartsWith("pylcall"))
            {
            ...
            // PyEval family
            if ([opcode
                    hasPrefix:@"pyeval"] || //(gCurWord.StartsWith("pyeval") ||
                [opcode
                    hasPrefix:@"pyleval"]) // gCurWord.StartsWith("pyleval"))
            {
            ...
            // PyExec family
            if ([opcode
                    hasPrefix:@"pyexec"] || //(gCurWord.StartsWith("pyexec") ||
                [opcode
                    hasPrefix:@"pylexec"]) // gCurWord.StartsWith("pylexec"))
            {
            ...
            // Pyrun family
            if ([opcode
                    hasPrefix:@"pyrun"] ||    //(gCurWord.StartsWith("pyrun") ||
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:mFile]) {
                ...
                // HelpBrowser.Navigate(Application.StartupPath +
                // "\\Help\\error.html"); MenuViewShowHelp_Click(null, null);
        ...
        if (path != @"") {
            // If it is not an opcode we show the Opcodes list reference part of
            // the manual
...
- (IBAction)wxHelpShowTutorials:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL URLWithString:@"http://www.csounds.com/tutorials"]];
}

- (IBAction)wxHelpShowFlossManual:(id)sender {
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL URLWithString:@"http://booki.flossmanuals.net/csound/"]];
}
...
- (IBAction)wxWinXoundTest:(id)sender {
    @try {
        ...
        NSString *input =
            [[NSBundle mainBundle] pathForResource:@"WinXound TEST"
                                            ofType:@"csd"];
        ...
        NSString *output =
            [NSString stringWithFormat:@"%@/WinXound TEST.csd", desktopPath];
        ...
        if ([fileManager fileExistsAtPath:output]) {
            ...
            [[NSDocumentController sharedDocumentController]
                openDocumentWithContentsOfURL:[NSURL fileURLWithPath:output]
                                      display:YES
                                        error:&error];
...
- (IBAction)wxHelpFindNext:(id)sender {
    ...
    if ([HelpWindow isVisible])
        [helpBrowser searchFor:[helpSearchField stringValue]
                     direction:YES
                 caseSensitive:false
                          wrap:true];
}
...
- (void)webView:(WebView *)sender
    ...
    @try {
        NSString *filename =
            [[[[frame provisionalDataSource] request] URL] path];
        ...
        NSLog(@"wxMainController -> didStartProvisionalLoadForFrame Error: %@ "
              @"- %@",
              [e name], [e reason]);
    }
}
...
- (id)tableView:(NSTableView *)tableView
    ...
    @try {
        if ([[[NSDocumentController sharedDocumentController]
                recentDocumentURLs] count] > 0) {
            NSURL *urlPath = [[[NSDocumentController sharedDocumentController]
                recentDocumentURLs] objectAtIndex:row];
    ...
    @catch (NSException *e) {
        NSLog(@"wxMainController -> objectValueForTableColumn Error: %@ - %@",
              [e name], [e reason]);
    }
...
- (bool)UpdateCabbage:(NSString *)filename {
    ...
    // return [self SendCabbageMessage:[NSString
    // stringWithFormat:@"CABBAGE_UPDATE| %@", filename]];
...
- (bool)CabbageExportVSTI:(NSString *)filename {
    // return [self SendCabbageMessage:[NSString
    // stringWithFormat:@"CABBAGE_EXPORT_VSTI| %@", filename]];
    return [self SendCabbageMessage:@"CABBAGE_EXPORT_VSTI"
                       withFilename:filename];
}

- (bool)CabbageExportVST:(NSString *)filename {
    // return [self SendCabbageMessage:[NSString
    // stringWithFormat:@"CABBAGE_EXPORT_VST| %@", filename]];
    return [self SendCabbageMessage:@"CABBAGE_EXPORT_VST"
                       withFilename:filename];
}

- (bool)CabbageExportAU:(NSString *)filename {
    // return [self SendCabbageMessage:[NSString
    // stringWithFormat:@"CABBAGE_EXPORT_AU| %@", filename]];
...
- (bool)SendCabbageMessage:(NSString *)message
    ...
    // Open and use the fifo as you would any file in Cocoa, but remember that
    // it's a FIFO
    // NSFileHandle* fifoOUT = [NSFileHandle
    // fileHandleForWritingAtPath:@"/tmp/cabbage_in"];
    // fileHandleForUpdatingAtPath //fileHandleForWritingAtPath

    // if (fifoOUT == nil && [[NSFileManager defaultManager]
    // fileExistsAtPath:@"/tmp/cabbage_out"])
    ...
    if (![[NSFileManager defaultManager]
        ...
        if ([[NSFileManager defaultManager]
            ...
            while (true) {
                ...
                if ((int)[previousDate timeIntervalSinceNow] <= -3)
                    break;
            }
        } else {
    ...
    // fifoOUT = [[NSFileHandle alloc] initWithFileDescriptor:fd closeOnDealloc:
    // YES];
    ...
    if (fifoOUT != nil) {
        ...
        NSString *str =
            [NSString stringWithFormat:@"%@| %@", message,
                                       filename]; // message; //@"test_string";
        UInt32 str_length = [str length];
        NSData *magic2 = [NSData dataWithBytes:&str_length
        ...
        NSMutableData *dataToWrite =
            [[NSMutableData alloc] initWithData:magic1]; // autorelease];
    ...
    if (fifoIN == nil &&
        [[NSFileManager defaultManager] fileExistsAtPath:@"/tmp/cabbage_out"]) {
        // fifoIN = [[NSFileHandle
        // fileHandleForReadingAtPath:@"/tmp/cabbage_out"] retain];
        const char *path = "/tmp/cabbage_out";
        int fd = open(path, O_RDONLY | O_NDELAY); // O_RDWR
        fifoIN = [[NSFileHandle alloc] initWithFileDescriptor:fd
                                               closeOnDealloc:YES]; // retain];
...
- (void)dataReady:(NSNotification *)n {
    ...
    if ([d length]) {
        ...
        if ([data rangeOfString:@"CABBAGE_FILE_UPDATED|"].location !=
            NSNotFound) {
            ...
            // if([[NSFileManager defaultManager] fileExistsAtPath:filename])
            {
                for (wxDocument *doc in
                     [[NSDocumentController sharedDocumentController]
                         documents]) {
                    NSLog(@"Document name: |%@| - |%@|", [[doc fileURL] path],
                          filename);
                    NSLog(@"Document length: %d - FileName length: %d",
                          [[[doc fileURL] path] length], [filename length]);

                    // if([[[doc fileURL] path] isEqualToString:filename])
                    if ([filename rangeOfString:[[doc fileURL] path]]
                            .location != NSNotFound) {
...
// BOOL success = [myDoc writeToURL:[self docURL] ofType:@"html"
// error:&theError];
//
// if (success == NO) {
...
//  	[NSString stringWithFormat:@"%@.csd",[[url path]
