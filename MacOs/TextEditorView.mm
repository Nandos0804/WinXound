...
//--------------------------------------------------------------------------------------------------
// TextEditorView: the main text area view with splitted window and other useful
// stuffs
//--------------------------------------------------------------------------------------------------
...
- (id)initWithFrame:(NSRect)frameRect {
    ...
    const void *sharedDocPointer =
        [textView1 getReferenceProperty:SCI_GETDOCPOINTER parameter:0];
    ...
    //[self setPosition:[self maxPossiblePositionOfDividerAtIndex: 0]
    //ofDividerAtIndex:0];
    ...
    [textView1 setGeneralProperty:SCI_MARKERDEFINE
                        parameter:0
                            value:SC_MARK_ARROW]; // SC_MARK_BACKGROUND
                                                  // //SC_MARK_FULLRECT
    [textView1 setGeneralProperty:SCI_SETMARGINTYPEN
                        parameter:0
                            value:SC_MARGIN_NUMBER];
    [textView1 setGeneralProperty:SCI_SETMARGINTYPEN
                        parameter:1
                            value:SC_MARGIN_SYMBOL];
    [textView1 setGeneralProperty:SCI_SETMARGINTYPEN parameter:2 value:0];
    [textView1 setGeneralProperty:SCI_SETMARGINWIDTHN
                        parameter:0
                            value:SCI_WIDTH_MARGINS];
    ...
    [textView2 setGeneralProperty:SCI_MARKERDEFINE
                        parameter:0
                            value:SC_MARK_ARROW]; // SC_MARK_BACKGROUND
    [textView2 setGeneralProperty:SCI_SETMARGINTYPEN
                        parameter:0
                            value:SC_MARGIN_NUMBER];
    [textView2 setGeneralProperty:SCI_SETMARGINTYPEN
                        parameter:1
    ...
    [textView2 setGeneralProperty:SCI_SETMARGINWIDTHN
                        parameter:0
                            value:SCI_WIDTH_MARGINS];
    ...
    // Attivare con la versione 2.02 assieme a SCI_SetAdditionalCaretsVisible =
    // 0 (false)
    [textView1 setGeneralProperty:SCI_SETVIRTUALSPACEOPTIONS
                        parameter:1
                            value:0];
    [textView2 setGeneralProperty:SCI_SETVIRTUALSPACEOPTIONS
                        parameter:1
                            value:0];
    [textView1 setGeneralProperty:SCI_SETADDITIONALCARETSVISIBLE
                        parameter:0
                            value:0];
    [textView2 setGeneralProperty:SCI_SETADDITIONALCARETSVISIBLE
                        parameter:0
                            value:0];
...
//	//[[NSNotificationCenter defaultCenter]
//postNotificationName:TextEditorUpdateUINotification object:self];
//}
//
...
//	//[[NSNotificationCenter defaultCenter]
//postNotificationName:TextEditorModifiedNotification object:self];
//}
...
- (void)SCI_ZOOM_Notification:(NSNotification *)notification {
    ...
    NSFont *tempFont = [NSFont fontWithName:[font familyName]
                                       size:[font pointSize] + zoom];

    SCI_WIDTH_MARGINS = [self measureTextWidth:SCI_MEASURE_STRING
                                          font:tempFont];

    // NSLog(@"%d",SCI_WIDTH_MARGINS);

    if ([sender getGeneralProperty:SCI_GETMARGINWIDTHN parameter:0] > 0) {
        [sender setGeneralProperty:SCI_SETMARGINWIDTHN
                         parameter:0
                             value:SCI_WIDTH_MARGINS];
    }
}
...
- (void)SCI_MOD_CONTAINER_Notification:(NSNotification *)notification {
    ...
    // NSLog(@"TextEditorView: SCI_MOD_CONTAINER - Real:%d Scintilla:%d",
    // eolModeReal, eolModeScintilla);
}
...
- (void)dealloc {
    ...
    [[NSNotificationCenter defaultCenter]
        removeObserver:self]; // Remove ALL Previous Notifications registration
                              // !!!
    ...
    // RELEASE TWO TIMES BECAUSE INNERVIEW RETAIN SCINTILLA (so one release for
    // innerview and one for scintillaView) NSLog(@"TextView retain count: %d",
    // [textView2 retainCount]);
    NSLog(@"START DEALLOC - TextView retain count1: %d - retain count2: %d",
          [textView1 retainCount], [textView2 retainCount]);
    [textView2 release];
    [textView2 release];
    textView2 = nil;
    ...
    NSLog(@"END DEALLOC - TextView retain count1: %d - retain count2: %d",
          [textView1 retainCount], [textView2 retainCount]);
}
...
- (NSString *)getFontName:(NSInteger)styleNumber {
    ...
    [[self getFocusedEditor] setReferenceProperty:SCI_STYLEGETFONT
...
- (NSInteger)measureTextWidth:(NSString *)text font:(NSFont *)font {
    // return (NSInteger)[self backend]->WndProc(SCI_TEXTWIDTH, STYLE_DEFAULT,
    // (sptr_t)text);
    CGSize textSize = NSSizeToCGSize([text
...
- (void)setSearchFlags:(NSInteger)searchflags {
    [textView1 setGeneralProperty:SCI_SETSEARCHFLAGS
                        parameter:searchflags
                            value:0];
    [textView2 setGeneralProperty:SCI_SETSEARCHFLAGS
                        parameter:searchflags
                            value:0];
}
...
- (NSInteger)searchInTarget:(NSString *)text {
    ...
    return [textView1 backend]->WndProc(SCI_SEARCHINTARGET, [text length],
                                        (sptr_t)rawValue);

    // NSInteger ret = [textView1 backend]->WndProc(SCI_SEARCHINTARGET, [text
    // length], (sptr_t)rawValue); delete[] rawValue; return ret;
}
...
- (void)setColorProperty:(int)property fromHTML:(NSString *)fromHTML {
    if ([fromHTML length] > 3 && [fromHTML characterAtIndex:0] == '#') {
        ...
        unsigned rawRed;
        [[NSScanner scannerWithString:[NSString stringWithUTF8String:value]]
            scanHexInt:&rawRed];
...
- (void)setScrollWidth:(NSInteger)pixelWidth {
    [textView1 setGeneralProperty:SCI_SETSCROLLWIDTH
                        parameter:pixelWidth
                            value:0];
    [textView2 setGeneralProperty:SCI_SETSCROLLWIDTH
                        parameter:pixelWidth
                            value:0];
}
// SCI_GETSCROLLWIDTH
...
- (void)setScrollWidthTracking:(BOOL)tracking {
    [textView1 setGeneralProperty:SCI_SETSCROLLWIDTHTRACKING
                        parameter:tracking
                            value:0];
    [textView2 setGeneralProperty:SCI_SETSCROLLWIDTHTRACKING
                        parameter:tracking
                            value:0];
}
...
- (void)setSelFore:(BOOL)useSel htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_SETSELFORE
                      parameter:useSel
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_SETSELFORE
                      parameter:useSel
                       fromHTML:htmlColor];
}
- (void)setSelBack:(BOOL)useSel htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_SETSELBACK
                      parameter:useSel
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_SETSELBACK
                      parameter:useSel
                       fromHTML:htmlColor];
}

- (void)StyleSetFore:(NSInteger)index htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_STYLESETFORE
                      parameter:index
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_STYLESETFORE
                      parameter:index
                       fromHTML:htmlColor];
}
- (void)StyleSetBack:(NSInteger)index htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_STYLESETBACK
                      parameter:index
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_STYLESETBACK
                      parameter:index
                       fromHTML:htmlColor];
}
...
- (void)StyleSetItalic:(NSInteger)index italic:(BOOL)italic {
    [textView1 setGeneralProperty:SCI_STYLESETITALIC
                        parameter:index
                            value:italic];
    [textView1 setGeneralProperty:SCI_STYLESETITALIC
                        parameter:index
...
- (NSFont *)getTextEditorFont // OK
{
    ...
    fontsize = (NSInteger)[textView1 getGeneralProperty:SCI_STYLEGETSIZE
...
{
    for (int i = 0; i < 34; i++) {
        [textView1 setStringProperty:SCI_STYLESETFONT
                           parameter:i
                               value:[newfont displayName]];
        [textView2 setStringProperty:SCI_STYLESETFONT
                           parameter:i
                               value:[newfont displayName]];
        [textView1 setGeneralProperty:SCI_STYLESETSIZE
                            parameter:i
                                value:[newfont pointSize]];
        [textView2 setGeneralProperty:SCI_STYLESETSIZE
                            parameter:i
                                value:[newfont pointSize]];
    }
    ...
    // NSFont* font = [self getTextEditorFont];
    NSFont *tempFont =
        [NSFont fontWithName:[newfont familyName]
                        size:[newfont pointSize] + [self getZoom]];
    SCI_WIDTH_MARGINS = [self measureTextWidth:SCI_MEASURE_STRING
                                          font:tempFont];
...
- (void)setShowSpaces:(BOOL)val // OK
{
    ...
    if (val == true) {
        [textView1 setGeneralProperty:SCI_SETVIEWWS
                            parameter:SCWS_VISIBLEALWAYS
                                value:0];
        [textView2 setGeneralProperty:SCI_SETVIEWWS
                            parameter:SCWS_VISIBLEALWAYS
                                value:0];
    } else {
...
{
    // Perform(mDirectPointer, SciConst.SCI_SETVIEWEOL, (Int32)(visible ? 1 :
    // 0), 0);
    [textView1 setGeneralProperty:SCI_SETVIEWEOL
                        parameter:(val ? 1 : 0)value:0];
    [textView2 setGeneralProperty:SCI_SETVIEWEOL
                        parameter:(val ? 1 : 0)value:0];
}

- (BOOL)getEndAtLastLine // OK
{
    // return Convert.ToBoolean(Perform(mDirectPointer,
    // SciConst.SCI_GETENDATLASTLINE, 0, 0));
...
- (void)setEndAtLastLine:(BOOL)val // OK
{
    ...
    [textView1 setGeneralProperty:SCI_SETENDATLASTLINE
                        parameter:(val ? 1 : 0)value:0];
    [textView2 setGeneralProperty:SCI_SETENDATLASTLINE
                        parameter:(val ? 1 : 0)value:0];
}
...
{
    // SCI_WIDTH_MARGINS = [textView1 measureTextWidth:SCI_MEASURE_STRING
    // font:[self getTextEditorFont]]; MEASURE TEXT WIDTH AND SET MARGINS (FOR
    // LINE NUMBERS)
    NSFont *font = [self getTextEditorFont];
    NSFont *tempFont = [NSFont fontWithName:[font familyName]
                                       size:[font pointSize] + [self getZoom]];
    SCI_WIDTH_MARGINS = [self measureTextWidth:SCI_MEASURE_STRING
                                          font:tempFont];
...
{
    // return Convert.ToBoolean(Perform(mDirectPointer,
    // SciConst.SCI_GETCARETLINEVISIBLE, 0, 0));
...
- (void)setMarkCaretLine:(BOOL)val // OK
{
    ...
    [textView1 setGeneralProperty:SCI_SETCARETLINEVISIBLE
                        parameter:(val ? 1 : 0)value:0];
    [textView2 setGeneralProperty:SCI_SETCARETLINEVISIBLE
                        parameter:(val ? 1 : 0)value:0];
}
...
- (void)setReadOnly:(BOOL)val // OK
{
    ...
    [textView1 setGeneralProperty:SCI_SETREADONLY
                        parameter:val ? 1 : 0
                            value:0];
    [textView2 setGeneralProperty:SCI_SETREADONLY
                        parameter:val ? 1 : 0
                            value:0];
}
...
- (void)SplitVertical {
    ...
    @try {
        ...
        NSLog(@"TextEditorView -> SplitVertical Error: %@ - %@", [e name],
              [e reason]);
    }
}
...
- (void)RemoveSplit // OK
{
    ...
    @try {
        ...
        NSLog(@"TextEditorView -> RemoveSplit Error: %@ - %@", [e name],
              [e reason]);
    }
}
...
- (void)SaveFile:(NSString *)filename {
    NSError *error = nil;
    BOOL ok = [[self getText] writeToFile:filename
                               atomically:YES
                                 encoding:NSUTF8StringEncoding
                                    error:&error]; // NSUnicodeStringEncoding
...
{
    [textView1 setStringProperty:SCI_SETLEXERLANGUAGE
                       parameter:0
                           value:languageName];
    [textView2 setStringProperty:SCI_SETLEXERLANGUAGE
                       parameter:0
                           value:languageName];
}
...
- (void)setKeyWords:(NSInteger)keyWordSet // OK
    ...
    [textView1 setReferenceProperty:SCI_SETKEYWORDS
                          parameter:keyWordSet
                              value:keyWordSetCHAR];
    [textView2 setReferenceProperty:SCI_SETKEYWORDS
                          parameter:keyWordSet
                              value:keyWordSetCHAR];
...
{
    // return
    // GetFocusedEditor.LineFromPosition(GetFocusedEditor.GetCurrentPos());
    NSInteger curPosition = 0;
    curPosition = [[self getFocusedEditor] getGeneralProperty:SCI_GETCURRENTPOS
                                                    parameter:0];

    return [[self getFocusedEditor] getGeneralProperty:SCI_LINEFROMPOSITION
                                             parameter:curPosition];
}

- (NSInteger)getLineLength:(NSInteger)lineNumber {
    // return [[self getFocusedEditor] getGeneralProperty:SCI_LINELENGTH
...
- (NSInteger)getCaretPosition {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETCURRENTPOS
                                             parameter:0];
}

- (void)setCaretPosition:(NSInteger)position {
    [[self getFocusedEditor] setGeneralProperty:SCI_GOTOPOS
                                      parameter:position
                                          value:0];
}
...
{
    [[self getFocusedEditor] setGeneralProperty:SCI_GOTOLINE
                                      parameter:lineNumber
                                          value:0];
}

- (void)InsertText:(NSInteger)position text:(NSString *)text {
    // CONVERT EOLS in order to match default Eol
    // text = [textView1 backend]->ConvertEOL(text);

    //[textView1 setStringProperty:SCI_INSERTTEXT parameter:position
    //value:text];
    [[self getFocusedEditor] setStringProperty:SCI_INSERTTEXT
                                     parameter:position
                                         value:text];
}
...
- (void)AddText:(NSString *)text {
    ...
    [[self getFocusedEditor] setStringProperty:SCI_ADDTEXT
                                     parameter:length
                                         value:text];
}
...
- (void)AppendText:(NSString *)text {
    ...
    [[self getFocusedEditor] setStringProperty:SCI_APPENDTEXT
                                     parameter:length
                                         value:text];
}
...
{
    @try {
        unichar KeyDec = [textView1 getGeneralProperty:SCI_GETCHARAT
                                             parameter:position];
        // KeyDec > 32 ??
        ...
        NSLog(@"TextEditorView -> getCharAt Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (NSInteger)getPositionFromPoint:(NSPoint)point {
    return [[self getFocusedEditor] getGeneralProperty:SCI_POSITIONFROMPOINT
                                             parameter:point.x
                                                 extra:point.y];
}
...
- (NSInteger)getPositionFromPointClose:(NSPoint)point {
    return
        [[self getFocusedEditor] getGeneralProperty:SCI_POSITIONFROMPOINTCLOSE
                                          parameter:point.x
                                              extra:point.y];
}
...
- (NSPoint)getQuotesPosition:(NSInteger)position withQuote:(NSString *)quote {
    ...
    for (NSInteger c = mCaretPosition - 1; c >= mLineStart; c--) {
...
- (NSString *)getTextInQuotes:(NSInteger)position withQuote:(NSString *)quote {
    ...
    if ((quotesPosition.y - quotesPosition.x) > 0 && quotesPosition.x > 0) {
        // mString = this.GetText().Substring(mStart, mEnd - mStart);
        // return mString;
        mString = [self
            getTextRange:NSMakeRange(quotesPosition.x,
                                     (quotesPosition.y - quotesPosition.x))];
        return mString;
    }
...
- (void)setFirstVisibleLine:(NSInteger)lineNumber {
    if ([[textView2 window] firstResponder] == [textView2 content]) {
        ...
        [self setFirstVisibleLineAtView:lineNumber view:1];
}

- (void)setFirstVisibleLineAtView:(NSInteger)lineNumber view:(NSInteger)view {
    NSInteger firstVisibleLine = 0;
    if (view == 2) {
        firstVisibleLine = [textView2 getGeneralProperty:SCI_GETFIRSTVISIBLELINE
                                               parameter:0];

        [textView2 setGeneralProperty:SCI_GOTOLINE
                            parameter:lineNumber
                                value:0];
        //[textView2 setGeneralProperty:SCI_LINESCROLL parameter:0
        //value:(lineNumber - firstVisibleLine)];
        [textView2 setGeneralProperty:SCI_SETFIRSTVISIBLELINE
                            parameter:lineNumber
                                value:0];
        [self setFocusOnSecondaryView];
    } else {
        firstVisibleLine = [textView1 getGeneralProperty:SCI_GETFIRSTVISIBLELINE
                                               parameter:0];

        [textView1 setGeneralProperty:SCI_GOTOLINE
                            parameter:lineNumber
                                value:0];
        //[textView1 setGeneralProperty:SCI_LINESCROLL parameter:0
        //value:(lineNumber - firstVisibleLine)];
        [textView1 setGeneralProperty:SCI_SETFIRSTVISIBLELINE
                            parameter:lineNumber
                                value:0];
        [self setFocusOnPrimaryView];
    }
...
- (NSInteger)getFirstVisibleLine {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETFIRSTVISIBLELINE
                                             parameter:0];
}

- (NSInteger)getFirstVisibleLineAtView:(NSInteger)view {
    if (view == 2) {
        return [textView2 getGeneralProperty:SCI_GETFIRSTVISIBLELINE
                                   parameter:0];
    } else {
        return [textView1 getGeneralProperty:SCI_GETFIRSTVISIBLELINE
                                   parameter:0];
    }
}

- (NSInteger)getLinesOnScreen {
    return [[self getFocusedEditor] getGeneralProperty:SCI_LINESONSCREEN
                                             parameter:0];
}
...
- (void)PerformPaste {
    //[[self getFocusedEditor] setGeneralProperty:SCI_PASTE parameter:0
    //value:0];
...
- (void)PerformSelectAll {
    [[self getFocusedEditor] setGeneralProperty:SCI_SELECTALL
                                      parameter:0
                                          value:0];
}
...
- (void)setZoom:(NSInteger)zoom {
    [[self getFocusedEditor] setGeneralProperty:SCI_SETZOOM
                                      parameter:zoom
                                          value:0];
}
               ...
               useRegEx:(BOOL)useRegEx;
{
    ...
    if ([self getTextLength] > 0) {
        ...
        NSInteger mStart =
            [self getCaretPosition]; // GetFocusedEditor.GetCurrentPos();
        NSInteger mEnd =
            [self getTextLength]; // GetFocusedEditor.GetTextLength();
        ...
        if (IsBackward) {
            // Search backward
            mStart = [self getCaretPosition] -
                     1; // GetFocusedEditor.GetCurrentPos() - 1;
            mEnd = 0;
        }
        ...
        @try {
            ...
            do {
                //[[self getFocusedEditor] setTargetStart:mStart];
                ////GetFocusedEditor.SetTargetStart(mStart);
                //[[self getFocusedEditor] setTargetEnd:mEnd];
                ////GetFocusedEditor.SetTargetEnd(mEnd);
                [self setTargetStart:mStart];
                [self setTargetEnd:mEnd];

                // mFindPos = [[self getFocusedEditor]
                // searchInTarget:StringToFind];
                // //GetFocusedEditor.SearchInTarget(StringToFind);
...
- (NSMutableArray *)SearchTextEx:(NSString *)text
    ...
    if ([self getTextLength] > 0) {
        ...
        do {
            //			mFindPos = this.FindText(StringToFind, MatchWholeWord,
            //MatchCase, 									 IsBackward, false, false, SkipRem, 									 mStart,
            //this.GetTextLength());
...
- (void)ReplaceTarget:(NSInteger)offset
    ...
    [self setTargetEnd:(offset + length)]; // GetFocusedEditor.SetTargetEnd(offset
                                           // + length);

    // GetFocusedEditor.ReplaceTarget(-1, ReplaceString);
    //[[self getFocusedEditor] setReferenceProperty:SCI_REPLACETARGET
    //parameter:length value:ReplaceString];
    [[self getFocusedEditor] setStringProperty:SCI_REPLACETARGET
                                     parameter:-1
                                         value:ReplaceString];
}

- (void)ReplaceText:(NSString *)replaceString {
    if ([[self getSelectedText] length] > 0 && [self getSelectedText] != nil) {
    ...
    do {
        ...
        if (mFindPos > -1) {
            ...
            if (mStart == 0) {
                ...
                // NSRunAlertPanel(@"WinXound Find and Replace",
                //				[NSString stringWithFormat:@"%d occurence(s)
                //replaced", mTotalOcc],
                //				@"OK",
                //				nil, nil);
                return mTotalOcc;
...
- (NSInteger)getLineNumberFromPosition:(NSInteger)position {
    // NSLog(@"getLineNumberFromPosition: %d",[[self getFocusedEditor]
    // getGeneralProperty:SCI_LINEFROMPOSITION parameter:position]);
    return [textView1 getGeneralProperty:SCI_LINEFROMPOSITION
                               parameter:position];
}

- (NSInteger)getPositionFromLineNumber:(NSInteger)linenumber {
    return [textView1 getGeneralProperty:SCI_POSITIONFROMLINE
                               parameter:linenumber];
}
...
- (NSInteger)getWordStart:(NSInteger)position {
    // return [textView1 backend]->WndProc(SCI_WORDSTARTPOSITION, position,
    // true);
...
- (NSInteger)getWordEnd:(NSInteger)position {
    ...
    NSInteger EndPos = 0;
    NSString *c = nil;
    for (EndPos = position; EndPos < [self getTextLength]; EndPos++) {
...
- (NSString *)getWordAt:(NSInteger)position {
    ...
    @try {
        ...
        NSLog(@"TextEditorView -> getWordAt Error: %@ - %@", [e name],
              [e reason]);
    }
...
- (BOOL)isValidChar:(NSString *)c {
    ...
    return ([c isEqualToString:@"#"] ||
            ...
            [c isEqualToString:@"_"] || [c isEqualToString:@"$"]);
}
...
- (void)setSelectionStart:(NSInteger)start {
    [[self getFocusedEditor] setGeneralProperty:SCI_SETSELECTIONSTART
                                      parameter:start
                                          value:0];
}

- (void)setSelectionEnd:(NSInteger)end {
    [[self getFocusedEditor] setGeneralProperty:SCI_SETSELECTIONEND
                                      parameter:end
                                          value:0];
}

- (NSInteger)getSelectionStart {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETSELECTIONSTART
                                             parameter:0];
}

- (NSInteger)getSelectionEnd {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETSELECTIONEND
                                             parameter:0];
}
...
- (void)setSelectedText:(NSString *)text {
    [[self getFocusedEditor] setStringProperty:SCI_REPLACESEL
                                     parameter:0
                                         value:text];
}
...
- (NSInteger)GetStyleAt:(NSInteger)position {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETSTYLEAT
                                             parameter:position];
}
...
- (NSInteger)GetLineEndPosition:(NSInteger)linenumber {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETLINEENDPOSITION
                                             parameter:linenumber];
}
...
- (void)SelectLine:(NSInteger)linenumber
    SetAsFirstVisibleLine:(BOOL)SetAsFirstVisibleLine {
...
- (void)ScrollCaret {
    [[self getFocusedEditor] setGeneralProperty:SCI_SCROLLCARET
                                      parameter:0
                                          value:0];
}
...
- (NSString *)GetEolModeReport {
    ...
    NSInteger crOccurrences =
        [s length] - [[s stringByReplacingOccurrencesOfString:@"\n"
                                                   withString:@""] length];

    NSInteger lfOccurrences =
        [s length] - [[s stringByReplacingOccurrencesOfString:@"\r"
                                                   withString:@""] length];
...
- (void)ConvertEOL:(NSInteger)eolMode {
    ...
    //	[textView1 setGeneralProperty:SCI_CONVERTEOLS parameter:eolMode
    //value:0]; 	[textView1 setGeneralProperty:SCI_ENDUNDOACTION parameter:0
    //value:0];
    ...
    NSString *a = [[self getText] stringByReplacingOccurrencesOfString:@"\r\n"
                                                            withString:@"\n"];
    NSString *temp = [a stringByReplacingOccurrencesOfString:@"\r"
                                                  withString:@"\n"];
...
- (BOOL)GetViewEOL {
    return [[self getFocusedEditor] getGeneralProperty:SCI_GETVIEWEOL
                                             parameter:0];
}

// SCI_SETPASTECONVERTENDINGS(bool convert)
- (void)SetPasteConvertEndings:(BOOL)convert {
    [textView1 setGeneralProperty:SCI_SETPASTECONVERTENDINGS
                        parameter:convert ? 1 : 0
                            value:0];
    [textView2 setGeneralProperty:SCI_SETPASTECONVERTENDINGS
                        parameter:convert ? 1 : 0
                            value:0];
}
...
- (NSInteger)BraceMatch:(NSInteger)pos maxReStyle:(NSInteger)maxReStyle {
    // return [textView1 backend]->WndProc(SCI_WORDENDPOSITION, position, true);
    return [[self getFocusedEditor] backend]->WndProc(SCI_BRACEMATCH, pos,
                                                      maxReStyle);
}

- (void)BraceHiglight:(NSInteger)pos1 pos2:(NSInteger)pos2 {
    [[self getFocusedEditor] setGeneralProperty:SCI_BRACEHIGHLIGHT
                                      parameter:pos1
                                          value:pos2];
}
...
- (void)MarkerSetFore:(NSInteger)number htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_MARKERSETFORE
                      parameter:number
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_MARKERSETFORE
                      parameter:number
                       fromHTML:htmlColor];
}

- (void)MarkerSetBack:(NSInteger)number htmlcolor:(NSString *)htmlColor {
    [textView1 setColorProperty:SCI_MARKERSETBACK
                      parameter:number
                       fromHTML:htmlColor];
    [textView2 setColorProperty:SCI_MARKERSETBACK
                      parameter:number
                       fromHTML:htmlColor];
}

- (void)MarkerSetAlpha:(NSInteger)number
              intcolor:(NSInteger)intColor // intColor value = 0 - 256
{
    [textView1 setGeneralProperty:SCI_MARKERSETALPHA
                        parameter:number
                            value:intColor];
    [textView2 setGeneralProperty:SCI_MARKERSETALPHA
                        parameter:number
                            value:intColor];
}
...
- (void)MarkerDelete:(NSInteger)linenumber {
    ...
    [textView1 setGeneralProperty:SCI_MARKERDELETE
                        parameter:linenumber
                            value:0];
    [textView2 setGeneralProperty:SCI_MARKERDELETE
                        parameter:linenumber
                            value:0];
}
...
- (void)MarkerDeleteAll:(NSInteger)markerNumber {
    ...
    [textView1 setGeneralProperty:SCI_MARKERDELETEALL
                        parameter:markerNumber
                            value:0];
    [textView2 setGeneralProperty:SCI_MARKERDELETEALL
                        parameter:markerNumber
                            value:0];
}

- (NSInteger)MarkerNext:(NSInteger)lineStart markerMask:(NSInteger)markerMask {
    // return Perform(mDirectPointer, SciConst.SCI_MARKERNEXT, lineStart,
    // markerMask);
    return [textView1 getGeneralProperty:SCI_MARKERNEXT
                               parameter:lineStart
                                   extra:markerMask];
}

- (NSInteger)MarkerPrevious:(NSInteger)lineStart
                 markerMask:(NSInteger)markerMask {
    // return Perform(mDirectPointer, SciConst.SCI_MARKERPREVIOUS, lineStart,
    // markerMask);
    return [textView1 getGeneralProperty:SCI_MARKERPREVIOUS
                               parameter:lineStart
                                   extra:markerMask];
}
...
- (void)GoToNextBookmark {
    // Int32 mLine = GetFocusedEditor.MarkerNext(this.GetCurrentLineNumber() +
    // 1, 1);
    NSInteger mLine = [self MarkerNext:[self getCurrentLineNumber] + 1
                            markerMask:1];
    if (mLine > -1)
        // this.GoToLine(mLine);
        [self GoToLine:mLine];
...
- (void)GoToPreviousBookmark {
    // Int32 mLine = GetFocusedEditor.MarkerPrevious(this.GetCurrentLineNumber()
    // - 1, 1);
    NSInteger mLine = [self MarkerPrevious:[self getCurrentLineNumber] - 1
                                markerMask:1];
    if (mLine > -1)
        // this.GoToLine(mLine);
        [self GoToLine:mLine];
...
//	NSInteger mLine = [self getCurrentLineNumber];
//
//	if([[self getFocusedEditor] getGeneralProperty:SCI_MARKERGET
//parameter:mLine] > 0) //Bookmark already exist so we remove it
//	{
