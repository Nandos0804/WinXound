    ...
    if (mLastFileName != nil) {
        if (![mLastFileName isEqualToString:@""]) {
            ...
            if (mPlayer != nil) {
                ...
                // this.Text = mTitle + " [Total time: " +
                // DisplayTime(mPlayer.Duration) + "]";
                [PlayerWindow
                    setTitle:[NSString
                                 stringWithFormat:
                                     @"WinXound Player [%@]",
...
- (NSString *)DisplayTime:(NSInteger)seconds {
    ...
    NSString *tempString =
        [NSString stringWithFormat:@"%02i:%02i:%02i", HH, MM, SS];
...
- (IBAction)sliderMoved:(id)sender {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self
                                             selector:trackingEndedSelector
                                               object:sender];
...
- (IBAction)wxPlay:(id)sender {
    ...
    if (!ret &&
        [[ButtonPlay title] isEqualToString:@"Pause (P)"]) // Player is playing
    {
        [mPlayer pause];
...
- (IBAction)wxBrowse:(id)sender {
    ...
    result = [oPanel
        runModalForDirectory:nil // applicationDirectory //NSHomeDirectory()
                        file:nil
                       types:fileTypes];
...
- (void)sound:(NSSound *)sound
    didFinishPlaying:(BOOL)aBool // STOPPED
{
